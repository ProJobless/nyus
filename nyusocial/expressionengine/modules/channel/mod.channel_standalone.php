...
/**
 * EDITS BY URGENT CONTENT
 * Line 510

// We aren't using index.php and need to pass a url segment to filter activity according to recency

Replaced: $RET = (isset($_POST['RET'])) ? $_POST['RET'] : $this->EE->functions->fetch_current_uri();
With: $RET = $this->EE->functions->fetch_site_index();

 * Line 1020
// CEM 2013 04 13
// Safecracker requires a trailing slash when index.php is removed from the configuration
...
class Channel_standalone extends Channel {
	...
	function entry_form($return_form = FALSE, $captcha = '')
	{
		...
		// BEW 04-14-2013
		// Need to add an id number to the end of POSTed URL, which interferes with safecracker's hidden value assignment.
		// We can keep this value as the site's main index page for all uses of safecracker on this site, so we do that.
