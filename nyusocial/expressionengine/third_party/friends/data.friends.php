...
class Friends_data extends Addon_builder_data_friends
{
	...
	function get_friend_ids_from_member_id( $site_id = '', $member_id = '' )
	{
		...
		$query	= $this->EE->db->query(
			"SELECT friend_id
			 ...
			 WHERE 	site_id
			 IN 	(" . implode( ",", $site_id ) . ")
			 AND 	member_id = " . $this->EE->db->escape_str( $member_id )
		);
	...
	function get_group_data_from_group_id( $site_id = '', $group_id = '' )
	{
		...
		$sql	= "SELECT 		fg.group_id, fg.member_id, fg.name,
				   ...
				   WHERE 		fg.site_id = " . $this->EE->db->escape_str( $site_id ) . "
				   AND 			fg.group_id = " . $this->EE->db->escape_str( $group_id ) . "
				   LIMIT 		1";
	...
	function get_group_data_from_group_name( $site_id = '', $group_name = '' )
	{
		...
		$sql	= "SELECT 		fg.group_id, fg.member_id, fg.name,
				   ...
				   ON 			m.member_id = fg.member_id
				   WHERE 		fg.site_id = " . $this->EE->db->escape_str( $site_id ) . "
				   AND 			name = '".$this->EE->db->escape_str( $group_name )."'
				   LIMIT 		1";
	...
	function get_data_from_group_comment_id( $site_id = '', $comment_id = '' )
	{
		...
		$sql	= "SELECT 	*
				   ...
				   AND 		comment_id 	= " . $this->EE->db->escape_str( $comment_id );
	...
	function get_member_id_from_username( $site_id = '', $username = '' )
	{
		...
		if ( $this->EE->session->userdata('username') == $username )
		{
			$this->cached[$cache_name][$cache_hash] = $this->EE->session->userdata('member_id');
	...
	function member_of_group( $site_id = '', $member_id = '', $group_id = '' )
	{
		...
		$sql	= "SELECT 	COUNT(*) AS count
				   ...
				   WHERE 	fgp.site_id = " . $this->EE->db->escape_str( $site_id ) . "
				   AND 		fgp.member_id = " . $this->EE->db->escape_str( $member_id ) . "
				   AND 		fgp.group_id = " . $this->EE->db->escape_str( $group_id ) . "
				   AND 		fgp.accepted = 'y'
				   AND 		fgp.declined = 'n'
	...
	function time_to_check_referrals_for_invitee( $site_id = '', $member_id = '' )
	{
		...
		$member_id	= ( $member_id == '' ) ? $this->EE->session->userdata('member_id'): $member_id;
	...
	function time_to_check_referrals_for_inviter( $site_id = '', $member_id = '', $interval = 0 )
	{
		...
		$sql	= "SELECT 	COUNT(*) AS count
				   ...
				   AND 		member_id = " . $this->EE->db->escape_str( $member_id ) . "
				   AND 		action = 'inviter_referral_check'
				   AND 		entry_date >= ( UNIX_TIMESTAMP() - ( 60 * " . $this->EE->db->escape_str( $interval ) . " ) )";
