...
class Friends_ext extends Extension_builder_friends
{
	...
	function allowed_group($which = '')
	{
		...
		return ! ( ! isset($this->EE->session->userdata[$which]) OR $this->EE->session->userdata[$which] !== 'y');
	}
	...
	function comment_end( $data, $moderate, $comment_id )
	{
		...
		$entry_id	= $this->EE->security->xss_clean( $data['entry_id'] );
			 ...
			 $this->EE->input->get_post('friends_notification_template') != '' )
		{
			...
			$sql	= " SELECT 		fgp.member_id, fgp.group_id
						...
						WHERE 		fgep.entry_id = '" . $this->EE->db->escape_str( $entry_id ) . "'
	...
	function delete_entry()
	{
		...
		$post	= $this->EE->security->xss_clean( $_POST['delete'] );
		...
		{
			$sql .= " entry_id = '" . $this->EE->db->escape_str( $val ) . "' OR ";
		}
	...
	public function user_delete_account_end ($instance)
	{
		...
		else if ($this->EE->input->post('member_id') AND
				 is_numeric($this->EE->input->post('member_id')) AND
				 $this->EE->input->post('member_id') > 0)
		{
		...
		return $this->actions()->delete_members_from_friends($member_id);
	}
	...
	public function delete_non_existent_members ()
	{
		...
		$delete = $this->EE->input->post('delete');
		...
		return $this->actions()->delete_members_from_friends($member_ids);
	}
	...
	function entry_end( $entry_id, $data, $ping_message )
	{
		...
		$FG =& $this->EE->friends_groups;

		// ----------------------------------------
		//	Clean post
		// ----------------------------------------

		$_POST['friends_group_id']	= $this->EE->security->xss_clean( $_POST['friends_group_id'] );
		...
		foreach ( $groups as $key => $val )
		{
			...
			if ( isset( $members[$key] ) === FALSE OR
				 ...
				 in_array( $this->EE->session->userdata('member_id'), $members[$key] ) === FALSE ) continue;
	...
	function _filter( $hide_privates = FALSE, $entry_ids = array() )
	{
		...
		if ( $this->EE->session->userdata['member_id'] == 0 OR $hide_privates === TRUE )
		{
			// ----------------------------------------
		...
		$query	= $this->EE->db->query( $prohibit );
	...
	function filter_private ( $entry_id = '', $sql = '' )
	{
		...
		if ( isset( $this->EE->extensions->last_call ) === TRUE AND
		...
		if ( $this->check_yes($this->EE->TMPL->fetch_param('disable_friends_filter') ) )
		{
			return $sql;
		}
		...
		$hide_privates	= $this->check_yes($this->EE->TMPL->fetch_param('friends_hide_privates'));
