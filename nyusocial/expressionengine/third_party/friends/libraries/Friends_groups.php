 ...
 *
 * This class handles Groups functionality.
 *
...
class Friends_groups extends Friends
{
	...
	function _add_friends_to_group( $friends = array(), $prefs = array(), $owner_id = '', $group_id = '' )
	{
		...
		$owner_id	= ( $owner_id == '' ) ? $this->EE->session->userdata('member_id'): $owner_id;
		...
		if ( $this->data->get_member_id_from_group_id(
				$this->EE->config->item('site_id'), $this->group_id ) != $owner_id )
		{
		...
		$sql		= "SELECT 	member_id, accepted, declined,
					   ...
					   AND 		group_id = ".$this->EE->db->escape_str( $this->group_id ) . "
					   AND 		member_id
					   IN 		( " . implode( ',', $friends ) . " )";
		...
		foreach ( $query->result_array() as $row )
		{
			...
			if ( isset( $members[ $row['member_id'] ] ) === TRUE )
			{
					 ...
					 $members[ $row['member_id'] ][ 'request_accepted' ] == 'n' )
				{
						 ...
						 $prefs['notification_approve'] != '' )
					{
						...
						$email['from_email']							= $this->EE->session->userdata['email'];
						$email['from_name']								= $this->EE->session->userdata['screen_name'];
			...
			if ( isset( $members[ $row['member_id'] ] ) === FALSE )
			{
				...
				$this->EE->db->query( $this->EE->db->insert_string( 'exp_friends_group_posts', $arr ) );
	...
	function comment_notify( $entries = array(), $members = array(), $email = array(), $prefix = 'friends_' )
	{
		...
		$sql	= "SELECT 	*
				   ...
				   WHERE 	entry_id = " . $this->EE->db->escape_str( $entries[0] );
		...
		if ( empty( $email['extra']['group_id'] ) === FALSE )
		{
			if ( isset( $this->cache['groups'][$email['extra']['group_id']] ) === FALSE )
			{
				$sql	= "SELECT 	group_id,
						   ...
						   WHERE 	group_id = " . $this->EE->db->escape_str( $email['extra']['group_id'] ) . "
						   LIMIT 	1";
	...
	function _create_group( $arr = array(), $create_default = FALSE )
	{
		...
		$query	= $this->EE->db->query(
			"SELECT COUNT(*) AS count
			 ...
			 AND 	name = '" . $this->EE->db->escape_str( $arr['name'] ) . "'
		...
		$arr['entry_date']	= $this->EE->localize->now;
		$arr['site_id'] = $this->EE->config->item('site_id');

		//	----------------------------------------
		//	Insert group
		//	----------------------------------------

		$this->EE->db->query( $this->EE->db->insert_string( 'exp_friends_groups', $arr ) );

		$this->group_id	= $this->EE->db->insert_id();
	...
	function edit_group()
	{
		...
		{
			return $this->_fetch_error( lang('not_logged_in'), $this->EE->input->get_post('error_template') );
		}
		...
		$arr['name']					= $this->EE->input->get_post('friends_group_name');
		$arr['title']					= $this->EE->input->get_post('friends_group_title');
		$arr['description']				= $this->EE->input->get_post('friends_group_description');
		$arr['subject']					= $this->EE->input->get_post('friends_group_subject');
		$arr['message']					= $this->EE->input->get_post('friends_group_message');
		$arr['private']					= $this->EE->input->get_post('friends_group_private');
		$arr['notification_approve']	= $this->EE->input->get_post('friends_notification_approve');
		$arr['notification_invite']		= $this->EE->input->get_post('friends_notification_invite');
		$arr['notification_remove']		= $this->EE->input->get_post('friends_notification_remove');
		$arr['subject_approve']			= $this->EE->input->get_post('friends_subject_approve');
		$arr['subject_invite']			= $this->EE->input->get_post('friends_subject_invite');
		$arr['subject_remove']			= $this->EE->input->get_post('friends_subject_remove');
		$arr['error_template']			= $this->EE->input->get_post('friends_error_template');
		...
		if ( isset( $_POST['friends_member_id'] ) === TRUE )
		{
			...
			if ( $this->check_yes($this->EE->input->get_post('friends_remove')) )
			{
				$mode	= 'remove_friends';
			}
		...
		$template	= ( $this->EE->input->get_post('friends_group_template') !== FALSE ) ?
						$this->EE->input->get_post('friends_group_template') : '';
	...
	function _edit_group( $arr = array() )
	{
		...
		$arr['edit_date']	= $this->EE->localize->now;

		//	----------------------------------------
		//	Update group
		//	----------------------------------------

		$this->EE->db->query( $this->EE->db->update_string( 'exp_friends_groups', $arr, array( 'group_id' => $this->group_id ) ) );
		...
		{
			if ( $this->EE->input->get_post( 'friends_notification_' . $val ) !== FALSE AND
				 $this->EE->input->get_post( 'friends_notification_' . $val ) != '' )
			{
				$prefs[ 'notification_' . $val ]	= $this->EE->input->get_post( 'friends_notification_' . $val );
			}

			if ( $this->EE->input->get_post( 'friends_subject_' . $val ) !== FALSE AND
				 $this->EE->input->get_post( 'friends_subject_' . $val ) != '' )
			{
				$prefs[ 'subject_' . $val ]			= $this->EE->input->get_post( 'friends_subject_' . $val );
			}
		}
	...
	function edit_group_preferences()
	{
		...
		if ( $this->EE->session->userdata['member_id'] == '0' )
		{
			return $this->_fetch_error(
				lang('not_logged_in'),
				$this->EE->input->post('error_template')
			);
		}
		...
		$update_all	= $this->check_yes(
			$this->EE->input->get_post('update_all')
		);
		...
		foreach ( $this->group_prefs as $key => $val )
		{
			if ( $this->check_yes( $this->EE->input->get_post($key) ) )
			{
				$updates[$key]	= 'y';
			}
			elseif ( $this->check_no( $this->EE->input->get_post($key) ) )
			{
				$updates[$key]	= 'n';
			}
		}
	...
	function _entries ( $params = array() )
	{
		...
		if (APP_VER >= '2.4.0')
		{
			...
			//this gets called by build_sql_query anyway
		...
		if (APP_VER >= '2.4.0')
		{
			// parse out the pagination_links conditional
			if ($channel->pagination->paginate === TRUE)
			{
				$channel->pagination->template_data = $this->EE->functions->prep_conditionals(
					$channel->pagination->template_data,
					array('pagination_links' => $channel->pagination->page_links != '')
				);
			}
		...
		else
		{
			...
			{
				$channel->paginate_data = $this->EE->functions->prep_conditionals(
					$channel->paginate_data,
					array('pagination_links' => $channel->pagination_links != '')
				);
			}

			if (APP_VER < '2.0.0')
			{
				if ( preg_match( '/\/(\d+)P(\d+)/s', $channel->pagination_links, $match ) )
				{
					$channel->pagination_links	= preg_replace( '/\/(\d+)P(\d+)/s', "/$1/P$2", $channel->pagination_links );
					$channel->page_next			= preg_replace( '/\/(\d+)P(\d+)/s', "/$1/P$2", $channel->page_next );
					$channel->page_previous		= preg_replace( '/\/(\d+)P(\d+)/s', "/$1/P$2", $channel->page_previous );
				}
			}
	...
	function entry_notify( $entries = array(), $members = array(), $email = array(), $prefix = 'friends_' )
	{
		...
		$data['from_email']				= ( empty( $email['from_email'] ) === FALSE ) ?
											$email['from_email'] : $this->EE->config->item('webmaster_email');
		$data['from_name']				= ( empty( $email['from_name'] ) === FALSE ) ?
											$email['from_name'] : $this->EE->config->item('webmaster_name');
		...
		$sql	= "SELECT 		m.*, md.*
				   ...
				   LEFT JOIN 	exp_member_data md
				   ON 			md.member_id = m.member_id
				   WHERE 		m.member_id != " . $this->EE->db->escape_str( $this->EE->session->userdata('member_id') ) . "
				   AND 			m.member_id
				   IN 			('" . implode( "','", $members ) . "')";
	...
	function group_add()
	{
		...
		$this->notify	= ! $this->check_no( $this->EE->TMPL->fetch_param('notify') ) ;
		...
		if ( $this->EE->TMPL->fetch_param('friends_group_id') !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param('friends_group_id') ) === TRUE )
		{
			$this->group_id	= $this->EE->TMPL->fetch_param('friends_group_id');
		}
		elseif ( $this->EE->TMPL->fetch_param('group_name') !== FALSE AND
				 ( $group_id = $this->data->get_group_id_from_group_name(
						$this->EE->config->item('site_id'), $this->EE->TMPL->fetch_param('group_name') ) ) !== FALSE )
		{
		...
		//	Sent through POST?
		elseif ( $this->EE->input->post('friends_group_id') !== FALSE AND
		...
		elseif ( $this->dynamic === TRUE AND
				 ( $seg = array_search( 'group', $this->EE->uri->segments ) ) !== FALSE )
		{
			if ( ! empty( $this->EE->uri->segments[ $seg + 1 ] ) )
			{
				//	Numeric group id?

				if ( is_numeric( $this->EE->uri->segments[ $seg + 1 ] ) === TRUE )
				{
					$this->group_id	= $this->EE->uri->segments[ $seg + 1 ];
				}
				elseif ( ( $group_id = $this->data->get_group_id_from_group_name(
							$this->EE->config->item('site_id'), $this->EE->uri->segments[ $seg + 1 ] ) ) !== FALSE )
				{
					$this->group_id	= $group_id;
					unset( $group_id );
				}
		...
		if ( $this->EE->TMPL->fetch_param('member_id') !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param('member_id') ) === TRUE )
		{
			$this->member_id	= $this->EE->TMPL->fetch_param('member_id');
		}
		elseif ( $this->EE->TMPL->fetch_param('username') !== FALSE AND
				 $this->EE->TMPL->fetch_param('username') != '' AND
				 ( $member_id = $this->data->get_member_id_from_username(
					$this->EE->config->item('site_id'), $this->EE->TMPL->fetch_param('username') ) ) !== FALSE )
		{
			$this->member_id	= $member_id;
			unset($member_id);
		}
		elseif ( $this->dynamic === TRUE AND
				 ( $seg = array_search( 'member', $this->EE->uri->segments ) ) !== FALSE )
		{
			if ( ! empty( $this->EE->uri->segments[ $seg + 1 ] ) )
			{
				//	Numeric member id?
				...
				if ( is_numeric( $this->EE->uri->segments[ $seg + 1 ] ) === TRUE )
				{
					$this->member_id	= $this->EE->uri->segments[ $seg + 1 ];
				}
		...
		if ( $this->EE->TMPL->fetch_param('delete') == 'yes' )
		{
			$delete	= TRUE;
		}
		elseif ( $this->dynamic === TRUE AND array_search( 'delete', $this->EE->uri->segments ) !== FALSE )
		{
			$delete	= TRUE;
		}
		...
		if (
			$delete === TRUE
			AND	$this->EE->session->userdata('member_id') == $this->data->get_member_id_from_group_id(
					$this->EE->config->item('site_id'), $this->group_id )
			...
			)
		{
			...
			$prefs	= array(
				'notification_remove'	=> $this->EE->TMPL->fetch_param('notification_remove'),
				'subject_remove'		=> $this->EE->TMPL->fetch_param('subject_remove')
			);
		...
		elseif (
			...
			AND	(
				$this->member_id == 0
				OR $this->EE->session->userdata('member_id') == $this->member_id
				)
			)
		{
			//	----------------------------------------
			...
			$prefs	= array(
				'notification_leave'	=> $this->EE->TMPL->fetch_param('notification_leave'),
				'subject_leave'			=> $this->EE->TMPL->fetch_param('subject_leave')
			);
			...
			)
		{
			...
			$prefs	= array(
				'notification_invite'	=> $this->EE->TMPL->fetch_param('notification_invite'),
				'notification_approve'	=> $this->EE->TMPL->fetch_param('notification_approve'),
				'subject_invite'		=> $this->EE->TMPL->fetch_param('subject_invite'),
				'subject_approve'		=> $this->EE->TMPL->fetch_param('subject_approve')
			);
			...
			)
		{
			...
			$prefs	= array(
				'notification_request'	=> $this->EE->TMPL->fetch_param('notification_request'),
				'notification_accept'	=> $this->EE->TMPL->fetch_param('notification_accept'),
				'subject_request'		=> $this->EE->TMPL->fetch_param('subject_request'),
				'subject_accept'		=> $this->EE->TMPL->fetch_param('subject_accept'),
			);
	...
	function _group_delete( $group_ids = array() )
	{
		...
		$this->_update_owned_group_stats( $this->EE->session->userdata('member_id') );
	...
	function group_entries()
	{
		...
		if ( $this->_group_id() === TRUE )
		{
			$query	= $this->EE->db->query(
				"SELECT 	COUNT(*) AS count
				 ...
				 WHERE 		fg.private 		= 'n'
				 OR 		(	fgp.member_id = '" . $this->EE->db->escape_str( $this->EE->session->userdata['member_id'] ) . "'
								AND fgp.accepted = 'y'
								AND fgp.declined = 'n'
		...
		if ( $this->group_id != 0 )
		{
			$sql	.= " AND group_id = '" . $this->EE->db->escape_str( $this->group_id ) . "'";
		}
		...
		if ( $this->check_yes( $this->EE->TMPL->fetch_param('private') ) OR
			 $this->check_no(  $this->EE->TMPL->fetch_param('public') ) )
		{
			$sql	.= " AND private = 'y'";
		}
		else
		...
		if ( $this->group_id != 0 )
		{
			$this->EE->TMPL->template	= $this->_parse_group_data( $this->EE->TMPL->template, $this->group_id );
		}
	...
	function group_entry()
	{
		...
		{
			return $this->EE->functions->prep_conditionals( $tagdata, $cond );
		}
		...
		$query	= $this->EE->db->query(
			"SELECT group_id
			 FROM 	exp_friends_group_entry_posts
			 WHERE 	entry_id = '" . $this->EE->db->escape_str( $this->entry_id ) . "'
			 AND 	private = 'y'"
		);
	...
	function group_entry_add()
	{
		...
		foreach ( $group_ids as $group_id )
		{
			...
			if ( $this->EE->input->get_post('friends_notification_template') !== FALSE AND
				 $this->EE->input->get_post('friends_notification_template') != '' )
			{
				$email['notification_template']	= $this->EE->input->get_post('friends_notification_template');
				$email['subject']				= $this->EE->input->get_post('friends_subject');
				$email['from_email']			= $this->EE->session->userdata('email');
				$email['from_name']				= $this->EE->session->userdata('screen_name');
				$email['member_id']				= $this->EE->session->userdata('member_id');
				$email['extra']['group_id']		= $group_id;

				$sql	= "SELECT 	member_id
						   FROM 	exp_friends_group_posts
						   WHERE 	group_id = '" . $this->EE->db->escape_str( $this->group_id ) . "'
						   AND 		accepted = 'y'
						   AND 		declined = 'n'
						   AND 		request_accepted = 'y'
						   AND 		request_declined = 'n'
	...
	function _group_entry_remove( $entries = array() )
	{
		...
		if ( $this->EE->session->userdata('group_id') != 1 AND
			 $this->data->get_member_id_from_group_id(
				$this->EE->config->item('site_id'), $this->group_id ) != $this->EE->session->userdata('member_id') )
		{
			foreach ( $query->result_array() as $row )
			{
		...
		foreach ( $query->result_array() as $row )
		{
			if ( in_array( $row['entry_id'], $mine ) === FALSE AND
				 ...
				 $this->data->get_member_id_from_group_id( $this->EE->config->item('site_id'),
					$this->group_id ) != $this->EE->session->userdata('member_id') ) continue;

			$sql	= "DELETE FROM 	exp_friends_group_entry_posts
					   WHERE 		entry_id = " . $this->EE->db->escape_str( $row['entry_id'] ) . "
					   AND 			group_id = " . $this->EE->db->escape_str( $row['group_id'] ) . "
					   AND 			site_id = " . $this->EE->db->escape_str( $row['site_id'] );

			$this->EE->db->query( $sql );

			$deletes	+= $this->EE->db->affected_rows();
		}
	...
	function group_form()
	{
		$act	= $this->EE->functions->fetch_action_id('Friends', 'edit_group');
		...
		if ( $this->_group_id() === TRUE )
		{
			...
			$this->EE->TMPL->tagdata	= $this->EE->functions->prep_conditionals( $this->EE->TMPL->tagdata, $query->row_array() );
		...
		else
		{
			foreach ( array('friends_group_name', 'friends_group_title', 'friends_group_description') as $val )
			{
				...
				$this->EE->TMPL->tagdata	= str_replace( LD . $val . RD, '', $this->EE->TMPL->tagdata );
			}
		}
		...
		$this->EE->TMPL->tagdata	= $this->_groups( $this->EE->TMPL->tagdata );
		...
		$this->arr['form_id']							= ( $this->EE->TMPL->fetch_param('form_id') ) ?
																$this->EE->TMPL->fetch_param('form_id') : 'group_form';

		$this->arr['form_name']							= ( $this->EE->TMPL->fetch_param('form_name') ) ?
																$this->EE->TMPL->fetch_param('form_name') : 'group_form';

		$this->arr['return']							= ( $this->EE->TMPL->fetch_param('return') ) ?
																$this->EE->TMPL->fetch_param('return') : '';

		$this->arr['friends_group_template']			= ( $this->EE->TMPL->fetch_param('group_template') ) ?
															$this->EE->TMPL->fetch_param('group_template') : '';

		foreach ( $this->group_notifications as $val )
		{
			$this->arr['friends_notification_' . $val]	= ( $this->EE->TMPL->fetch_param('notification_'.$val) ) ?
															$this->EE->TMPL->fetch_param('notification_'.$val) : '';
			$this->arr['friends_subject_' . $val]			= ( $this->EE->TMPL->fetch_param('subject_'.$val) ) ?
															$this->EE->TMPL->fetch_param('subject_'.$val) : '';
		}
	...
	function _group_id()
	{
		...
		if ( $this->EE->input->get_post('friends_group_id') !== FALSE AND
			 is_array( $this->EE->input->get_post('friends_group_id') ) === FALSE AND
			 $this->_numeric( $this->EE->input->get_post('friends_group_id') ) === TRUE )
		{
			$this->group_id	= $this->EE->input->get_post('friends_group_id');
		...
		{
			if ( $this->_numeric( $this->EE->TMPL->fetch_param('friends_group_id') ) === TRUE )
			{
				$this->group_id	= $this->EE->TMPL->fetch_param('friends_group_id');

				return TRUE;
			}
			elseif ( $this->EE->TMPL->fetch_param('group_name') !== FALSE )
			{
				$query	= $this->EE->db->query(
					"SELECT group_id
					 FROM 	exp_friends_groups
					 WHERE 	name = '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('group_name') ) . "'"
				);
			...
			elseif ( preg_match( "#/".$this->trigger."/(\w+)/?#", $this->EE->uri->uri_string, $match ) )
			{
				$sql	= "SELECT 	group_id
						   FROM 	exp_friends_groups";
				...
				{
					$sql	.= " WHERE group_id = '" . $this->EE->db->escape_str( $match['1'] ) . "'";
				}
				else
				{
					$sql	.= " WHERE name = '" . $this->EE->db->escape_str( $match['1'] ) . "'";
				}
		...
		if ( $this->EE->uri->query_string != '' AND
			 $this->dynamic === TRUE )
		{
			...
			else
			{
				...
				if (preg_match("#^".$cat_segment."/#", $qstring, $match) AND $this->EE->TMPL->fetch_param('weblog'))
				{
	...
	function group_invite()
	{
		...
		if ( $this->EE->session->userdata['member_id'] == '0' )
		{
			return $this->_fetch_error( lang('not_logged_in'), $this->EE->input->post('error_template') );
		}
		...
		$sql	= "SELECT 		fgp.accepted, fg.name, fg.title,
				   ...
				   WHERE 		fgp.group_id = '" . $this->EE->db->escape_str( $this->group_id ) . "'
				   AND 			fgp.member_id = '" . $this->EE->db->escape_str( $this->EE->session->userdata('member_id') ) . "'";
		...
		$notification_template	= ( $this->EE->input->get_post('notification_template') !== FALSE AND
									$this->EE->input->get_post('notification_template') != '' ) ?
										$this->EE->input->get_post('notification_template') : '';
		...
		$this->notify	= ! $this->check_no( $this->EE->input->post('notify'));

		//	----------------------------------------
		//	Are we deleting?
		//	----------------------------------------

		if ( strpos( $this->EE->uri->uri_string, "/delete" ) !== FALSE OR
			  $this->check_yes($this->EE->input->post('delete') ) )
		{
			$this->delete	= TRUE;
		}
		...
		if ( $this->delete === TRUE )
		{
			...
			$email['message']	= str_replace(
				...
				array( $this->EE->session->userdata('screen_name'), $query->row('title') ),
	...
	function group_members()
	{
		...
		if ( preg_match( "/" . LD . "friends_paginate" . RD . "(.+?)" . LD .
				preg_quote(T_SLASH, '/') . "friends_paginate" . RD . "/s", $this->EE->TMPL->tagdata, $match ) )
		{
			$this->paginate_match	= $match;

			$this->EE->TMPL->tagdata	= str_replace( $this->paginate_match[0], '', $this->EE->TMPL->tagdata );
		}
		...
		if ( $this->EE->TMPL->fetch_param('type') !== FALSE AND
			 $this->EE->TMPL->fetch_param('type') != '' )
		{
			if ( $this->EE->TMPL->fetch_param('type') == 'invite' )
			{
				return $this->group_membership_invites();
			}
			elseif ( $this->EE->TMPL->fetch_param('type') == 'request' )
			{
	...
	function group_members_confirmed()
	{
		...
		$this->tagdata	= ( $this->tagdata == '' ) ? $this->EE->TMPL->tagdata: $this->tagdata;
	...
	function _fetch_group_members_through_subquery( $subquery = '' )
	{
		...
		{
			$sql	.= $this->EE->functions->sql_andor_string( $this->EE->TMPL->fetch_param('member_id'), 'm.member_id' );
		}
	...
	function groups ()
	{
		...
		return $this->_groups( $this->EE->TMPL->tagdata, $group_type, 'return_no_results' );
	}
	...
	function _groups( $tagdata = '', $group_type = 'owner', $return_no_results = '' )
	{
		...
		if ( $this->check_yes( $this->EE->TMPL->fetch_param('show_private_groups') ) )
		{
			$privacy	= "";
		}
		...
		switch ( $group_type )
		{
			...
			//	Owner
			case 'owner':
				$sql	.= " WHERE fg.member_id = " . $this->EE->db->escape_str( $this->member_id );
				$sql	.= $privacy;
				break;
			...
			case 'invite':
				$sql	.= " LEFT JOIN 	exp_friends_group_posts fgp
							 ...
							 AND 		fgp.request_declined = 'n'
							 $privacy
							 AND 		fgp.member_id = " . $this->EE->db->escape_str( $this->member_id );
				break;
		...
		$sql	.= " AND fg.site_id IN (" . implode( ',', $this->EE->TMPL->site_ids ) . ")";

		//	----------------------------------------
		//	Hide mine
		//	----------------------------------------

		if ( $this->check_yes( $this->EE->TMPL->fetch_param('hide_mine') ) )
		{
			$sql	.= " AND fg.member_id != '" . $this->EE->db->escape_str( $this->EE->session->userdata['member_id'] ) . "'";
		}
		...
		if ( $this->group_id != 0 )
		{
			$sql	.= " AND fg.group_id = '".$this->EE->db->escape_str( $this->group_id )."'";
		}
		...
		if ( strtolower($this->EE->TMPL->fetch_param('sort')) != 'asc' )
		{
			$sql	.= " DESC";
		}
		else
		...
		$query		= $this->EE->db->query( str_replace( "MDB", "COUNT(*) AS count", $sql ) );
		...
		$tagdata	= $this->EE->functions->prep_conditionals( $tagdata, array( 'friends_total_results' => $count ) );

		$tagdata	= $this->EE->TMPL->swap_var_single( "friends_total_results", $count, $tagdata );
		...
		$joined_groups	= $this->data->get_joined_groups_from_member_id(
						$this->EE->config->item( 'site_id' ), $this->EE->session->userdata( 'member_id' ) );
	...
	function member_of_group ()
	{
		...
		elseif ( $this->EE->TMPL->fetch_param('group_name') !== FALSE AND
				( $group_id = $this->data->get_group_id_from_group_name(
					$this->EE->config->item('site_id'),
					$this->EE->TMPL->fetch_param('group_name') ) ) !== FALSE )
		{
			$this->group_id	= $group_id;
			unset( $group_id );
		}
		...
		elseif ( $this->dynamic === TRUE AND ( $seg = array_search( 'group', $this->EE->uri->segments ) ) !== FALSE )
		{
			if ( ! empty( $this->EE->uri->segments[ $seg + 1 ] ) )
			{
				//	Numeric group id?
		...
		if ( $this->group_id == 0 )
		{
			return $tagdata	= $this->EE->functions->prep_conditionals( $tagdata, $cond );
		}
		...
		elseif ( $this->EE->input->post('friends_member_id') !== FALSE AND
				 is_numeric( $this->EE->input->post('friends_member_id') ) === TRUE )
		{
			$this->member_id	= $this->EE->input->post('friends_member_id');
		}
		elseif ( $this->dynamic === TRUE AND ( $seg = array_search( 'member', $this->EE->uri->segments ) ) !== FALSE )
		{
			if ( ! empty( $this->EE->uri->segments[ $seg + 1 ] ) )
			{
				//	Numeric member id?
		...
		if ( $this->data->member_of_group( $this->EE->config->item('site_id'), $this->member_id, $this->group_id ) === FALSE )
		{
			return $tagdata	= $this->EE->functions->prep_conditionals( $tagdata, $cond );
		}
		else
	 ...
	 * Parse group data
	 *
	 * This parses group variables on $this->EE->TMPL->template
	 *
	 * @access	private
	 * @return	string
	 */
			...
			else
			{
				$query	= $this->EE->db->query(
					"SELECT *
					 ...
					 WHERE 	site_id
					 IN 	(" . implode( ',', $this->EE->TMPL->site_ids ) . ")
					 AND 	group_id = " . $this->EE->db->escape_str( $group_id )
				);
	...
	function _request_membership_or_accept_invite( $prefs = array() )
	{
		...
		if ( $this->group_id == 0 OR ( $group_data = $this->data->get_group_data_from_group_id(
				$this->EE->config->item('site_id'), $this->group_id ) ) === FALSE )
		{
			$this->message[]	= lang('group_id_required');
			return FALSE;
		}
		...
		if ( $query->num_rows() == 0 )
		{
			...
			else
			{
				...
				$sql	= $this->EE->db->insert_string( 'exp_friends_group_posts', $data );
		...
		if ( $this->notify === TRUE AND $email['notification_template'] != '' )
		{
			...
			$email['extra']['friends_owner_username']		= $group_data['owner_username'];
			$email['extra']['friends_owner_member_id']		= $group_data['owner_member_id'];
			$email['extra']['friends_user_screen_name']		= $this->EE->session->userdata['screen_name'];
			$email['extra']['friends_user_username']		= $this->EE->session->userdata['username'];
			$email['extra']['friends_user_member_id']		= $this->EE->session->userdata['member_id'];
	...
	function _remove_friends_from_group( $friends = array(), $prefs = array() )
	{
		...
		$owner_id = $this->data->get_member_id_from_group_id($this->EE->config->item('site_id'), $this->group_id );
		...
		$sql	= "SELECT 		fgp.member_id, m.email, m.username, m.screen_name
				   ...
				   WHERE 		fgp.member_id
				   IN 			('" . implode( ',', $friends ) . "')
				   AND 			fgp.group_id = " . $this->EE->db->escape_str( $this->group_id );
	...
	function _remove_self_from_group( $prefs = array() )
	{
		...
		if ( $group_data['member_id'] == $this->EE->session->userdata('member_id') )
		{
		...
		if ( $this->notify === TRUE AND
			 $this->EE->db->affected_rows() > 0 AND
			 ...
			 $prefs['notification_leave'] != '' )
		{
			...
			$email['extra']['friends_group_id']				= $this->group_id;
			$email['extra']['friends_screen_name']			= $this->EE->session->userdata['screen_name'];
			$email['extra']['friends_group_name']			= $group_data['name'];
			$email['extra']['friends_group_title']			= $group_data['title'];
			$email['extra']['friends_group_description']	= $group_data['description'];
			$email['extra']['friends_owner_screen_name']	= $group_data['owner_screen_name'];
	 ...
	 */
	 ...
	 function _update_group_stats( $group_id = 0 )
	 {
		...
		$sql	= "SELECT 	COUNT(*) AS count
				   FROM 	exp_friends_group_entry_posts
				   WHERE 	group_id = " . $this->EE->db->escape_str( $group_id );
