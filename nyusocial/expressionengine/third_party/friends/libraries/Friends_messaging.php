 ...
 *
 * This class handles Messaging functionality.
 *
...
class Friends_messaging extends Friends
{
	...
	function message_delete()
	{
		...
		if ( $this->check_yes($this->EE->input->post('friends_ajax')) )
		{
			$this->ajax	= TRUE;
		}
		...
		if ( $this->EE->TMPL->fetch_param('message_id') !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param('message_id') ) === TRUE )
		{
			$message_id[]	= $this->EE->TMPL->fetch_param('message_id');
		}
		elseif ( preg_match( "/\/(\d+)/s", $this->EE->uri->uri_string, $match ) AND $this->dynamic === TRUE )
		{
			$message_id[]	= $match['1'];
		}
	...
	function _message_delete( $message_id = array() )
	{
		...
		$sql	= 'SELECT copy_id, message_id, message_folder, message_deleted FROM exp_message_copies WHERE recipient_id = '.$this->EE->db->escape_str( $this->EE->session->userdata('member_id') ).' AND message_id IN ('.implode( ',', $message_id ).')';
		...
		{
			$this->message[]	= str_replace( '%count%', $this->EE->db->affected_rows(), lang('messages_deleted') );
		}
	...
	function message_folder_edit()
	{
		...
		$folders['current']	= $this->data->get_message_folders_for_member( $this->EE->session->userdata('member_id') );
		...
		{
			$val = $this->EE->security->xss_clean($val);
		...
		if ( empty( $folders['deleted'] ) === FALSE  )
		{
			...
			$query	= $this->EE->db->query(
				"SELECT message_folder, message_deleted
				 ...
				 AND 	recipient_id = '" . $this->EE->db->escape_str( $this->EE->session->userdata('member_id') ) . "'
				 AND 	message_folder
				 NOT IN (1,2)
				 AND 	message_folder
			...
			if ( empty( $folders['deleted'] ) === FALSE )
			{
				$this->EE->db->query(
					"DELETE FROM 	exp_message_copies
					 WHERE 			recipient_id = " . $this->EE->db->escape_str( $this->EE->session->userdata('member_id') ) . "
	...
	{
		$act		= $this->EE->functions->fetch_action_id('Friends', 'message_folder_edit');
		...
		$folders	= $this->data->get_message_folders_for_member( $this->EE->session->userdata('member_id') );
		...
		if ( $this->dynamic === TRUE AND preg_match( "/\/(\d+)/s", $this->EE->uri->uri_string, $match ) )
		{
		...
		$this->arr['form_id']			= ( $this->EE->TMPL->fetch_param('form_id') ) ?
											$this->EE->TMPL->fetch_param('form_id') : 'friends_message_folder_form';

		$this->arr['form_name']			= ( $this->EE->TMPL->fetch_param('form_name') ) ?
											$this->EE->TMPL->fetch_param('form_name') : 'friends_message_folder_form';

		$this->arr['return']			= ( $this->EE->TMPL->fetch_param('return') ) ? $this->EE->TMPL->fetch_param('return'): '';
	...
	function message_folder_name()
	{
		...
		if ( $this->EE->TMPL->fetch_param('message_folder_id') !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param('message_id') ) === TRUE )
		{
			$folder_id	= $this->EE->TMPL->fetch_param('message_folder_id');
		}
		elseif ( $this->EE->TMPL->fetch_param('message_folder') !== FALSE AND
				 $this->EE->TMPL->fetch_param('message_folder') != '' )
		{
			$temp	= explode( "|", strtolower( $this->EE->TMPL->fetch_param('message_folder') ) );
	...
	function message_folders()
	{
		...
		$temp	= $this->data->get_message_folders_for_member( $this->EE->session->userdata('member_id') );
		...
		foreach ( $folders as $row )
		{
			...
			$tagdata	= $this->EE->functions->prep_conditionals( $this->EE->TMPL->tagdata, $row );
	...
	{
		$act		= $this->EE->functions->fetch_action_id('Friends', 'send_message');
		...
		$reply_mode		= $this->check_yes( $this->EE->TMPL->fetch_param('reply_mode') );

		//	----------------------------------------
		//	Reply mode
		//	----------------------------------------

		$forward_mode	= $this->check_yes( $this->EE->TMPL->fetch_param('forward_mode') );
		...
		{
			$member_id	= $this->EE->TMPL->fetch_param('friend_id');
		}
		elseif ( $this->EE->TMPL->fetch_param('message_id') !== FALSE AND
				( is_numeric( $this->EE->TMPL->fetch_param('message_id') ) === TRUE ) )
		{
			$message_id	= $this->EE->TMPL->fetch_param('message_id');
		}
		...
		elseif ( preg_match( "/".LD."friends".RD."(.*?)".LD.preg_quote(T_SLASH, '/')."friends".RD."/s", $this->EE->TMPL->tagdata, $match ) )
		{
			$this->tagdata	= $match['1'];
		...
		elseif ( preg_match( "/".LD."confirmed".RD."(.*?)".LD.preg_quote(T_SLASH, '/')."confirmed".RD."/s", $this->EE->TMPL->tagdata, $match ) )
		{
			$this->tagdata	= $match['1'];
		...
		elseif ( preg_match( "/".LD."requests".RD."(.*?)".LD.preg_quote(T_SLASH, '/')."requests".RD."/s", $this->EE->TMPL->tagdata, $match ) )
		{
			$this->tagdata	= $match['1'];
		...
		if ( ! empty( $message_id )  )
		{
			$sql	= "SELECT 		md.message_date			AS friends_message_date,
					   ...
					   AND 			mc.recipient_id = " . $this->EE->db->escape_str( $this->EE->session->userdata('member_id') ) . "
					   AND 			mc.message_id = " . $this->EE->db->escape_str( $message_id ) . "
					   LIMIT 		1";
		...
		$this->arr['form_id']			= ( $this->EE->TMPL->fetch_param('form_id') ) ?
											$this->EE->TMPL->fetch_param('form_id'): 'message_form';

		$this->arr['form_name']			= ( $this->EE->TMPL->fetch_param('form_name') ) ?
											$this->EE->TMPL->fetch_param('form_name'): 'message_form';

		$this->arr['return']			= ( $this->EE->TMPL->fetch_param('return') ) ? $this->EE->TMPL->fetch_param('return'): '';

		$this->arr['link']				= ( $this->EE->TMPL->fetch_param('link') ) ? $this->EE->TMPL->fetch_param('link'): '';

		$this->arr['notification_template']	= ( $this->EE->TMPL->fetch_param('notification_template') ) ?
	...
	function message_move()
	{
		...
		if ( $this->EE->input->post('friends_ajax') !== FALSE AND $this->EE->input->post('friends_ajax') == 'yes' )
		{
			$this->ajax	= TRUE;
		}
		...
		if ( $this->EE->TMPL->fetch_param('message_folder_id') !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param('message_folder_id') ) === TRUE )
		{
			$folder_id	= $this->EE->TMPL->fetch_param('message_folder_id');
		}
		elseif ( preg_match( "/\/message_folder(\d+)/s", $this->EE->uri->uri_string, $match ) AND $this->dynamic === TRUE )
		{
			$folder_id	= $match['1'];
		}
	...
	function _message_move( $message_id = array(), $folder_id = '' )
	{
		...
		else
		{
			$this->message[]	= str_replace(
				array( '%count%', '%folder%' ),
				array( $this->EE->db->affected_rows(), $folder ),
				lang('messages_moved')
			);
		}
	...
	function messages()
	{
		...
		if ( preg_match_all( "/".LD."friends_message_folders".RD."(.*?)".LD.
							preg_quote(T_SLASH, '/')."friends_message_folders".RD."/s", $this->EE->TMPL->template, $matches ) )
		{
			foreach ( array_keys( $matches[0] ) as $m )
			{
				...
				foreach ( $folders as $key => $val )
				{
					...
					$tdata	= $this->EE->functions->prep_conditionals(
						$tdata,
						array(
				...
				$this->EE->TMPL->template	= str_replace( $matches['0'][$m], $r, $this->EE->TMPL->template );
			}
		}
		...
		if ( ! empty( $message_id ))
		{
			...
			foreach ( $query->result_array() as $row )
			{
				...
				{
					$row['friends_message_time_read']	= $this->EE->localize->now;
					$this->EE->db->query(
						$this->EE->db->update_string(
							'exp_message_copies',
							array(
								'message_received'	=> 'y',
								'message_read' 		=> 'y',
								'message_time_read' => $this->EE->localize->now
							),
							array(
								'copy_id' => $row['copy_id']
							)
				...
				$r	.= $this->_parse_message_data( $row, $this->EE->TMPL->tagdata );
			}
		...
		if ( $this->check_no($this->EE->TMPL->fetch_param('show_read')) )
		{
		...
		if ( $this->check_yes($this->EE->TMPL->fetch_param('show_sent')) )
		{
	...
	function _parse_message_data( $row = array(), $tagdata = '' )
	{
		...
		if ( strpos( $tagdata, LD.'friends_message'.RD ) !== FALSE )
		{
			...
			$formatting['text_format']		= ( $this->EE->TMPL->fetch_param('text_format') != '' ) ?
												$this->EE->TMPL->fetch_param('text_format') : 'none';
		}
								...
								preg_quote(T_SLASH, '/')."friends_message_recipients".RD."/s", $tagdata, $matches ) )
		{
			...
			$sql	= 'SELECT 	recipient_id
					   ...
					   AND 		message_id = ' . $this->EE->db->escape_str( $row['friends_message_id'] );
			...
			foreach ( array_keys( $matches[0] ) as $key )
			{
				$this->params		= $this->EE->functions->assign_parameters( $matches['1'][$key] );
	...
	function send_message()
	{
		...
		if ( ( $message_waiting_period = $this->data->get_preference_from_site_id(
				$this->EE->config->item( 'site_id' ), 'message_waiting_period' ) ) !== FALSE )
		{
			if ( $this->EE->session->userdata['join_date'] > ( $this->EE->localize->now - ( $message_waiting_period * 60 * 60 ) ) )
			{
		...
		if ( ( $message_throttling = $this->data->get_preference_from_site_id( $this->EE->config->item( 'site_id' ), 'message_throttling' ) ) !== FALSE )
		{
			$query	= $this->EE->db->query(
				"SELECT COUNT(*) AS count
				 FROM 	exp_message_data
				 WHERE 	sender_id = " . $this->EE->db->escape_str( $this->EE->session->userdata( 'member_id' ) ) . "
				 AND 	message_date >= " . ( $this->EE->localize->now - $message_throttling ) );
		...
		if ( ( $max_message_chars = $this->data->get_preference_from_site_id(
						$this->EE->config->item( 'site_id' ), 'max_message_chars' ) ) !== FALSE )
		{
		...
		$message	= $this->EE->security->xss_clean( $_POST['friends_message'] );
		...
		{
			$subject	= $this->EE->security->xss_clean( $_POST['friends_subject'] );
		}
		...
		if ( ( $message_day_limit = $this->data->get_preference_from_site_id(
				$this->EE->config->item( 'site_id' ), 'message_day_limit' ) ) !== FALSE )
		{
			$query	= $this->EE->db->query(
				"SELECT 	COUNT(*) AS count
				 ...
				 ON 		md.message_id = mc.message_id
				 WHERE 		md.sender_id = " . $this->EE->db->escape_str( $this->EE->session->userdata( 'member_id' ) ) . "
				 AND 		message_date >= " . ( $this->EE->localize->now - 86400 ) );
		...
		if ( count( $friends ) > 0 )
		{
			...
			/*$sql	= "SELECT 		m.*
					   ...
					   WHERE 		f.site_id = " . $this->EE->db->escape_str( $this->EE->config->item('site_id') ) . "
		...
		if ( count( $groups ) > 0 )
		{
			$sql	= "SELECT 		m.*
					   ...
					   WHERE 		fgp.site_id = " . $this->EE->db->escape_str( $this->EE->config->item('site_id') ) . "
					   AND 			m.accept_messages = 'y'
					   AND 			fgp.declined = 'n'
					   AND 			m.friends_opt_out = 'n'
					   AND 			fgp.group_id
		...
		unset( $friends[ $this->EE->session->userdata('member_id') ] );
		...
		if ( ( $max_recipients_per_message = $this->data->get_preference_from_site_id(
				$this->EE->config->item( 'site_id' ), 'max_recipients_per_message' ) ) !== FALSE )
		{
		...
		$data	= array(
			...
			'sender_id'				=> $this->EE->session->userdata( 'member_id' ),
			'message_authcode'		=> $this->EE->functions->random('alpha', 10)
		);
		...
		foreach ( $friends as $id => $row )
		{
			...
			$this->EE->db->query( $this->EE->db->insert_string( 'exp_message_copies', $data ) );
		}
		...
		$data['recipient_id']		= $this->EE->session->userdata( 'member_id' );
		...
		{
			$_POST['notification_template']	= $this->EE->security->xss_clean( $_POST['notification_template'] );
			...
			foreach ( $friends as $id => $row )
			{
				...
				$data['link']							= ( empty( $_POST['link'] ) === FALSE ) ?
															$this->EE->security->xss_clean( $_POST['link'] ) : '';
				$data['member_id']						= $id;
		...
		$this->_recount_private_messages(
			array_merge(
				array_keys( $friends ),
				array( $this->EE->session->userdata('member_id') )
			)
		);
