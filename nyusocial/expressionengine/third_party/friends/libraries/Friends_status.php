 ...
 *
 * This class handles Status functionality.
 *
...
class Friends_status extends Friends
{
	...
	function status()
	{
		...
		{
			$sql	.= " AND fs.member_id = ".$this->EE->db->escape_str( $this->member_id );
		}
		else
		{
			$friend_ids	= $this->data->get_friend_ids_from_member_id( $this->EE->TMPL->site_ids, $this->member_id );
	...
	function status_delete()
	{
		...
		if ( $this->EE->TMPL->fetch_param( 'status_id' ) !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param( 'status_id' ) ) === TRUE )
		{
		...
		if ( $this->data->get_member_id_from_status_id($status_id) != $this->EE->session->userdata('member_id') )
		{
		...
		if ($this->EE->extensions->active_hook('friends_status_delete_status') === TRUE)
		{
			$data	= $this->EE->extensions->universal_call( 'friends_status_delete_status', $this, $data );
		...
		$this->EE->db->query(
			"DELETE FROM 	exp_friends_status
			 WHERE 			status_id = " . $this->EE->db->escape_str( $status_id )
		);
	...
	{
		$act	= $this->EE->functions->fetch_action_id('Friends', 'status_update');
		...
		$this->arr['form_id']			= ( $this->EE->TMPL->fetch_param('form_id') ) ?
											$this->EE->TMPL->fetch_param('form_id') : 'friends_status_form';

		$this->arr['form_class']		= ( $this->EE->TMPL->fetch_param('form_class') ) ?
											$this->EE->TMPL->fetch_param('form_class') : '';

		$this->arr['form_name']			= ( $this->EE->TMPL->fetch_param('form_name') ) ?
											$this->EE->TMPL->fetch_param('form_name') : 'friends_status_form';

		$this->arr['return']			= ( $this->EE->TMPL->fetch_param('return') ) ?
											$this->EE->TMPL->fetch_param('return') : '';

		$this->arr['template']			= ( $this->EE->TMPL->fetch_param('template') ) ?
											$this->EE->TMPL->fetch_param('template') : '';

		//	----------------------------------------
		//	Parse member data
		//	----------------------------------------

		$tagdata	= $this->_parse_member_data( $this->member_id, $this->EE->TMPL->tagdata );
	...
	function status_update()
	{
		...
		$data['status_date']	= $this->EE->localize->now;
		$data['member_id']		= $this->member_id;
		$data['group_id']		= $this->EE->session->userdata('group_id');
		...
		if ($this->EE->extensions->active_hook('friends_status_update_status') === TRUE)
		{
			$data	= $this->EE->extensions->universal_call( 'friends_status_update_status', $this, $data );
			if ( $this->EE->extensions->end_script === TRUE ) exit();
		}
		...
		$this->EE->db->query( $this->EE->db->insert_string( 'exp_friends_status', $data ) );
