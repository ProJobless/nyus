...
class Friends_mcp extends Module_builder_friends
{
	...
	public function __construct( $switch = TRUE )
	{
		...
		elseif($this->version_compare($this->database_version(), '<', FRIENDS_VERSION) OR
				! $this->extensions_enabled())
		{
			// For EE 2.x, we need to redirect the request to Update Routine
			$_GET['method'] = 'friends_module_update';
		}
	}
	// END Friends_cp_base()
	...
	public function delete_friend_confirm($message = '')
	{
		...
		if ( empty( $_POST['toggle'] ) OR ! is_numeric($this->EE->input->post('member_id')))
		{
			return $this->members();
		}
		...
		$i								= 0;
		$this->cached_vars['item_id']	= $this->EE->input->post('member_id', TRUE);
		$this->cached_vars['items']		= array();

		foreach ( $this->EE->input->post('toggle') as $key => $val )
		{
			if ( is_numeric( $val ) )
			{
	...
	public function delete_friend ( $message = '' )
	{
		...
		$query	= $this->EE->db->query(
			"SELECT entry_id
			 FROM 	exp_friends
			 WHERE 	entry_id
			 IN 	('" . implode("','", $this->EE->db->escape_str($ids)) . "')"
		);
		...
		$qq = $this->EE->db->query("SELECT member_id, friend_id
						FROM exp_friends
						WHERE entry_id
						IN ('" . implode("','", $this->EE->db->escape_str($friendship_ids)) . "')");
		...
		if ( count( $sql ) > 0 )
		{
			...
			{
				require $this->addon_path.'mod.friends'.EXT;
			}
	...
	public function delete_group()
	{
		...
		foreach ( $query->result_array() as $row )
		{
			$sql[]	= "DELETE FROM 	exp_friends_groups
					   WHERE 		group_id = '" . $this->EE->db->escape_str($row['group_id']) . "'";
	...
	{
		if ( $this->EE->input->get_post('item_id') === FALSE OR $this->EE->input->get_post('item_id') == '' )
		{
			return $this->groups();
		}
		...
		$sql	= "SELECT 	fgp.member_id
				   ...
				   AND 		fgp.group_id = '".$this->EE->db->escape_str( $this->EE->input->get_post('item_id') )."'
				   AND 		fgp.member_id
				   NOT IN 	( 	SELECT 	member_id
								FROM 	exp_friends_groups
								WHERE 	group_id = '" . $this->EE->db->escape_str( $this->EE->input->get_post('item_id') ) . "'
							)
				   AND 		fgp.member_id
				   IN 		('" . implode("','", $this->EE->db->escape_str($ids)) . "')";
		...
		foreach ( $query->result_array() as $row )
		{
			$sql[]	= "DELETE FROM 	exp_friends_group_posts
					   ...
					   AND 			group_id = '" . $this->EE->db->escape_str( $this->EE->input->get_post('item_id') ) . "'";
		}
		...
		$this->EE->friends_groups->_update_group_stats(
			$this->EE->input->get_post('item_id')
		);
	...
	public function delete_group_member_confirm()
	{
		...
		$this->cached_vars['item_id']	= $this->EE->input->get_post('group_id');
		$this->cached_vars['items']	= array();
	...
	{
	   $group_id			= ( ! in_array($this->EE->input->get_post('group_id'), array(FALSE, '', 0, '0'), TRUE) AND
								is_numeric($this->EE->input->get_post('group_id'))) ?
		...
		if ( ! $group_name = $this->EE->input->get_post('group_name') )
		{
			return $this->EE->output->show_user_error('general', lang('group_name_required'));
		}

		if ( ! $group_title = $this->EE->input->get_post('group_title') )
		{
			$group_title	= $group_name;
		}

		$group_description	= $this->EE->input->post('group_description');
		$group_private		= ( $this->EE->input->post('group_private') !== FALSE ) ?
								$this->EE->input->post('group_private') : 'n';
		...
		$sql	= "SELECT 	group_id, name
				   FROM 	exp_friends_groups
				   WHERE 	name = '" . $this->EE->db->escape_str($group_name) . "'";

		if ( $update )
		{
			$sql .= " AND group_id != '" . $this->EE->db->escape_str( $group_id ) . "'";
		}
		...
		$this->EE->friends_groups->_update_group_stats($group_id);
	...
	{
		$edit_group_mode							= ( $this->EE->input->get_post('group_id') ) ?
		...
		if ( $this->EE->input->get_post('group_id') !== FALSE )
		{
			$sql	= "SELECT 	*
					   FROM 	exp_friends_groups
					   WHERE 	group_id = '" . $this->EE->db->escape_str( $this->EE->input->get_post('group_id') ) . "'
					   LIMIT 	1";
	...
	public function group_invitees($message = '')
	{
		...
		$this->cached_vars['group_id']		= $this->EE->input->get_post('group_id');
		$this->cached_vars['group_title']	= '';
		...
		$sql	= "SELECT 		%q
				   ...
				   WHERE 		fgp.group_id = '" . $this->EE->db->escape_str($this->EE->input->get_post('group_id')) . "'
				   AND 			fgp.accepted = 'n'
		...
		foreach ( $query->result_array() as $row )
		{
			$row['date']						= $this->EE->localize->set_human_time( $row['entry_date'] );
	...
	public function member( $message = '' )
	{
		...
		if ( ( $member_id	= $this->EE->input->get_post('member_id') ) === FALSE )
		{
			return $this->index();
		}
		...
		$sql	=  "SELECT 		%q
					...
					WHERE 		f.member_id = " . $this->EE->db->escape_str( $member_id ) . "
					AND 		f.friend_id != 0
					ORDER BY 	m.screen_name
					ASC";
		...
		if ( $query->row('count') > $this->row_limit )
		{
			$row_count			= ( $this->EE->input->get_post('row') === FALSE ) ? 0 : $this->EE->input->get_post('row');
		...
		$query	= $this->EE->db->query( str_replace( '%q', 'f.*, m.screen_name, m.total_friends', $sql ) );
	...
	public function preferences( $message = '' )
	{
		...
		foreach ( $prefs as $key => $val )
		{
			$pref						= $this->data->get_preference_from_site_id(
												$this->EE->config->item( 'site_id' ), $key );
		...
		if ( $this->EE->input->post('max_message_chars') !== FALSE )
		{
			// -------------------------------------
			//	Prep vars
			// -------------------------------------
			...
			foreach ( $prefs as $key => $val )
			{
				if ( $this->EE->input->get_post($key) !== FALSE )
				{
					...
					if( stristr($key, 'message') AND is_numeric( $this->EE->input->get_post($key) ) !== TRUE) {continue;}
					$prefs[ $key ]				= $this->EE->db->escape_str( $this->EE->input->get_post($key) );
					$this->cached_vars[$key]	= $this->EE->input->get_post($key);
				}
			}
	...
	function _build_right_link($lang_line, $link)
	{
		...
		$this->EE->cp->set_right_nav($ee2_links);
	}
