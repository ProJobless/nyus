...
class Friends extends Module_builder_friends
{
	...
	public function __call($method = '', $args = array())
	{
		foreach ($this->lib_functions as $lib => $methods)
		{
			...
			{
				$this->EE->load->library($lib);
				return call_user_func_array(
					array($this->EE->{$lib}, $method),
					$args
				);
			}
		}
	...
	public function __construct()
	{
		...
		if ($this->database_version() == FALSE OR
			$this->version_compare($this->database_version(), '<', FRIENDS_VERSION) OR
			! $this->extensions_enabled())
		{
			$this->disabled = TRUE;

			trigger_error(lang('friends_module_disabled'), E_USER_NOTICE);
		}
		...
		if ( isset($this->EE->TMPL) AND
			 ...
			 $this->EE->TMPL->fetch_param('trigger') != '' )
		{
			$this->trigger	= $this->EE->TMPL->fetch_param('trigger');
		}
		elseif ( empty( $this->EE->uri->segments['2'] ) === FALSE )
		{
			$this->trigger	= $this->EE->uri->segments['2'];
		}

		//sets the member ID if it is there and set to current_user
		if ( isset($this->EE->TMPL) AND
			 isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND
			 $this->EE->TMPL->fetch_param('member_id') === 'CURRENT_USER' AND
			 ( isset($this->EE->session) AND
			   is_object($this->EE->session) AND
			   isset($this->EE->session->userdata))
		   )
		{
		...
		$this->clean_site_id = $this->EE->db->escape_str( $this->EE->config->item( 'site_id' ) );

		$this->EE->load->helper(array('string','text'));
	}
	// END Friends()
	...
	function add( $local = FALSE )
	{
		...
		if ( $this->EE->input->post('ajax') !== FALSE AND $this->EE->input->post('ajax') == 'yes' )
		{
			$this->ajax	= TRUE;
		}
		...
		$this->notify	= ! $this->check_no( $this->EE->input->post('friends_notify'));
		...
		if ( $this->EE->input->post('friends_member_id') !== FALSE )
		{
			if ( is_array( $this->EE->input->post('friends_member_id') ) === TRUE )
			{
				$members	= $this->EE->input->post('friends_member_id');
			}
			else
			{
				$members[]	= $this->EE->input->post('friends_member_id');
		...
		if ( preg_match( "#/"."username"."/(\w+)/?#", $this->EE->uri->uri_string, $match ) AND $this->dynamic === TRUE )
		{
			$members[]	= $this->data->get_member_id_from_username( $this->EE->config->item( 'site_id' ), $match[1] );
		}
		...
		if ( strpos( $this->EE->uri->uri_string, "/block" ) !== FALSE )
		{
		...
		elseif ( strpos( $this->EE->uri->uri_string, "/delete" ) !== FALSE )
		{
	...
	function _add( $members = array() )
	{
		...
		{
			if ( $this->EE->input->get_post( 'friends_' . $val ) !== FALSE AND
				 $this->EE->input->get_post( 'friends_' . $val ) != '' )
			{
				$$val	= $this->EE->input->get_post( 'friends_' . $val );
			}
			elseif ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND
					 $this->EE->TMPL->fetch_param( $val ) !== FALSE AND
					 $this->EE->TMPL->fetch_param( $val ) != '' )
			{
		...
		if ( count( $members ) == 0 )
		{
			...
			$confirmq	= $this->EE->db->query(
				"SELECT block
				 FROM 	exp_friends
				 WHERE 	site_id = {$this->clean_site_id}
				 AND 	member_id = " . $this->EE->db->escape_str( $this->member_id ) . "
				 AND 	friend_id = " . $this->EE->db->escape_str( $this->EE->session->userdata['member_id'] ) . "
				 LIMIT 	1"
			);
		...
		if ( strpos( $this->EE->uri->uri_string, "/private" ) !== FALSE OR $this->EE->input->get_post('private') == 'y' )
		{
			$private	= 'y';
		}
		...
		foreach ( $query->result_array() as $row )
		{
			if ( $row['friends_opt_out'] != 'y' )
			{
				...
				else
				{
					...
					$this->EE->db->query( $this->EE->db->insert_string('exp_friends', $data) );
				}
					...
					$this->EE->db->query(
						$this->EE->db->update_string(
							...
							array(
								...
								'friend_id' => $this->EE->session->userdata['member_id']
							)
						)
					);
	...
	function _block( $members = array() )
	{
		...
		if ( count( $members ) == 0 )
		{
			...
			if ( $query->num_rows() > 0 )
			{
				...
				$ended_friendships	= $this->EE->db->affected_rows();
			}
			else
			{
				$this->EE->db->query(
	...
	function _check_form_hash()
	{
		if ( ! $this->check_secure_forms())
		{
			return $this->_fetch_error(
				lang('not_authorized'),
				$this->EE->input->get_post('template')
			);
		}
		...
		if ( count( $members ) == 0 )
		{
			...
			$query = $this->EE->db->query(
				"SELECT COUNT(*) AS count
				 FROM 	exp_friends
				 WHERE 	friend_id = '".$this->EE->db->escape_str($this->member_id)."'
				 AND 	member_id = '".$this->EE->db->escape_str($this->EE->session->userdata['member_id'])."'
				 LIMIT  1"
			);
		...
		else
		{
			...
			$query		= $this->EE->db->query(
				"SELECT entry_id
				 FROM 	exp_friends
				 WHERE 	friend_id
				 IN 	('" . implode( "','", $this->EE->db->escape_str($members) ) . "')
				 AND 	member_id = '" . $this->EE->db->escape_str($this->EE->session->userdata['member_id']) . "'"
			);
	...
	function _entry_id( $type = 'entry_id' )
	{
		...
		if ( $this->_numeric( trim( $this->EE->TMPL->fetch_param( $type ) ) ) === TRUE )
		{
			$this->$type	= trim( $this->EE->TMPL->fetch_param( $type ) );

			return TRUE;
		}
		elseif ( $this->EE->uri->query_string != '' OR
				 ( isset( $this->EE->uri->page_query_string ) === TRUE AND
				   $this->EE->uri->page_query_string != '' AND
				   $type == 'entry_id' )
			   )
		{
			...
			else
			{
				...
				if (preg_match("#^".$cat_segment."/#", $qstring, $match) AND $this->EE->TMPL->fetch_param($this->sc->channel))
				{
					...
					{
						$xsql	= $this->EE->functions->sql_andor_string(
								$this->EE->TMPL->fetch_param($this->sc->channel),
				...
				if ( $type == 'member_id' )
				{
					...
					$sql	= "SELECT 	member_id
							   FROM 	exp_members
							   WHERE 	username = '" . $this->EE->db->escape_str( $qstring ) . "'";
				...
				else
				{
					...
					$sql	= "SELECT {$this->sc->db->titles}.entry_id
							   ...
							   AND    {$this->sc->db->titles}.url_title = '" . $this->EE->db->escape_str($qstring) . "'";
	...
	function _fetch_members_through_subquery( $subquery1 = '', $subquery2 = '' )
	{
		...
		if ( $this->EE->TMPL->fetch_param( 'member_group_id' ) )
		{
			$sql	.= $this->EE->functions->sql_andor_string(
				$this->EE->TMPL->fetch_param( 'member_group_id' ),
				'm.group_id'
			);
		}
		...
		{
			$sql	.= " AND m.email = '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('email') ) . "'";
		}
		...
		if ( is_numeric( $this->EE->TMPL->fetch_param('days') ) )
		{
			$days	= $this->EE->localize->now - ( $this->EE->TMPL->fetch_param('days') + 86400 );

			$sql	.= " AND m.join_date >= '" . $this->EE->db->escape_str( $days ) . "'";
		}
		elseif ( is_numeric( $this->EE->TMPL->fetch_param('hours') ) )
		{
			$hours	= $this->EE->localize->now - ( $this->EE->TMPL->fetch_param('hours') + 3600 );

			$sql	.= " AND m.join_date >= '" . $this->EE->db->escape_str( $hours ) . "'";
		}
		...
		if ( preg_match( "/\/(username|screen_name)\/(.+?)\//s", $this->EE->uri->uri_string, $match ) AND $this->dynamic === TRUE )
		{
			$sql	.= ( $match['1'] == 'username' ) ? " AND m.username LIKE '": " AND m.screen_name LIKE '";
			$sql	.= $this->EE->db->escape_str( $match['2'] ) . "%'";
		}
		elseif ( $this->EE->TMPL->fetch_param('search_username') )
		{
			$sql	.= " AND m.username LIKE '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('search_username') ) . "%'";
		}
		elseif ( $this->EE->TMPL->fetch_param('search_screen_name') )
		{
			$sql	.= " AND m.screen_name LIKE '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('search_screen_name') ) . "%'";
		}
		...
		if ( $this->EE->TMPL->fetch_param('orderby') !== FALSE AND
			 $this->EE->TMPL->fetch_param('orderby') != '' AND
			 strpos( $this->EE->TMPL->fetch_param('orderby'), '|' ) === FALSE )
		{
			if( strtolower($this->EE->TMPL->fetch_param('orderby')) == 'random' )
			{
				$is_random = TRUE;
				$sql .= " ORDER BY RAND()";
			}
		...
		{
			$this->friends_count++;
			$r	.= $this->_parse_member_data( $id, $this->EE->TMPL->tagdata );
		}
	...
	function _fetch_template( $tmp = '', $data = array(), $settings = array() )
	{
		if ( $tmp == '' )
		{
			if ( ! $tmp = $this->EE->input->get_post('template') )
			{
				if ( ! (isset($this->EE->TMPL) AND is_object($this->EE->TMPL)) OR ! $tmp = $this->EE->TMPL->fetch_param('template') )
				{
					return FALSE;
				}
			}
		...
		{
			return $this->EE->output->show_user_error('general', array(lang('template_loop')));
		}
	...
	function form()
	{
		$act	= $this->EE->functions->fetch_action_id('Friends', 'update');
		...
		if ( preg_match( "/".LD."members".RD."(.*?)".LD.preg_quote(T_SLASH, '/')."members".RD."/s", $this->EE->TMPL->tagdata, $match ) )
		{
			$this->tagdata	= $match['1'];
		}
		elseif ( preg_match( "/".LD."invites".RD."(.*?)".LD.preg_quote(T_SLASH, '/')."invites".RD."/s", $this->EE->TMPL->tagdata, $match ) )
		{
			$invite_mode	= TRUE;
			$this->tagdata	= $match['1'];
		}
		...
		elseif ( $this->EE->TMPL->fetch_param('type') !== FALSE AND $this->EE->TMPL->fetch_param('type') == 'mine' )
		{
		...
		$tagdata	= str_replace( $match['0'], $this->tagdata, $this->EE->TMPL->tagdata );
		...
		$this->arr['RET']							= $this->EE->input->post('RET') ?
														$this->EE->input->post('RET') :
														$this->EE->functions->fetch_current_uri();

		$this->arr['form_id']						= $this->EE->TMPL->fetch_param('form_id') ?
														$this->EE->TMPL->fetch_param('form_id') :
														'friends_form';

		$this->arr['form_name']						= $this->EE->TMPL->fetch_param('form_name') ?
														$this->EE->TMPL->fetch_param('form_name') :
														'friends_form';
		$this->arr['return']						= $this->EE->TMPL->fetch_param('return') ?
		...
		{
			$this->arr['friends_' . $item]			= $this->EE->TMPL->fetch_param($item) ?
															$this->EE->TMPL->fetch_param($item) : '';
		}
	...
	function _wall_form( $data = array() )
	{
		...
		$arr	= array(
			...
			'onsubmit'		=> ( $this->EE->TMPL->fetch_param('onsubmit') ) ?
									$this->EE->TMPL->fetch_param('onsubmit') : ''
		);

		if ( $this->EE->TMPL->fetch_param('name') !== FALSE )
		{
			$arr['name']	= $this->EE->TMPL->fetch_param('name');
		}
		...
		if ($this->check_yes($this->EE->TMPL->fetch_param('secure_action')))
		{
			if (isset($arr['action']))
			{
		...
		if ($this->check_yes($this->EE->TMPL->fetch_param('secure_return')))
		{
			foreach(array('return', 'RET') as $return_field)
			{
				if (isset($arr['hidden_fields'][$return_field]))
				{
					...
					{
						$arr['hidden_fields'][$return_field] = $this->EE->functions->create_url(
	...
	function _form()
	{
		...
		$r	= $this->EE->functions->form_declaration(
			array(
	...
	{
		$act	= $this->EE->functions->fetch_action_id('Friends', 'invite_friends');
		...
		$this->arr['RET'] =  $this->EE->functions->fetch_current_uri();
		...
		{
			$this->arr[ $val ] = ( $this->EE->TMPL->fetch_param( $val ) ) ? $this->EE->TMPL->fetch_param( $val ) : '';
		}
		...
		{
			$this->arr[ 'friends_' . $val ]	= ( $this->EE->TMPL->fetch_param( $val ) ) ? $this->EE->TMPL->fetch_param( $val ): '';
		}
	...
	{		//	----------------------------------------
		...
		if ( $this->EE->input->post('friends_group_id') !== FALSE AND
			 is_numeric( $this->EE->input->post('friends_group_id') ) === TRUE )
		{
			$this->group_id	= $this->EE->input->post('friends_group_id');
		}
		...
		$this->notify	= ! $this->check_no($this->EE->input->get_post('friends_notify'));
		...
		$template	= ( $this->EE->input->get_post('friends_template') !== FALSE ) ?
						$this->EE->input->get_post('friends_template') : '';
	...
	{		//	----------------------------------------
		...
		if ( ! $email = $this->EE->input->post('friends_emails') )
		{
		...
		$email	= $this->EE->security->xss_clean( $email );
		$email	= trim( $email );
		...
		if ( count( $members ) > 0 )
		{
			...
			if ( $this->group_id != 0 )
			{
				...
				{
					if ( $this->EE->input->get_post($key) !== FALSE AND $this->EE->input->get_post($key) != '' )
					{
						$prefs[$val]	= $this->EE->input->get_post($key);
					}
				}
		...
		if ( count( $emails ) > 0 )
		{
			...
			foreach ( $emails as $email )
			{
				...
				$data	= array(
					...
					'email'			=> $email,
					'member_id'     => $this->EE->session->userdata['member_id'],
					'referrer_id'	=> $this->EE->session->userdata['member_id'],
				...
				$notification_template	= $this->EE->input->get_post('friends_notification_invite') ?
											$this->EE->input->get_post('friends_notification_invite') : '';
				...
				if ( $notification_template != '' )
				{
					...
					$data['from_email']				= $this->EE->session->userdata['email'];
					$data['from_name']				= $this->EE->session->userdata['screen_name'];
					$data['subject']				= $this->EE->input->post('friends_subject_invite') ?
														$this->EE->input->post('friends_subject_invite') : '';
	...
	function invites()
	{
		...
		if ( $this->EE->TMPL->fetch_param('invite_type') !== FALSE AND
			 $this->EE->TMPL->fetch_param('invite_type') != '' AND
			 $this->EE->TMPL->fetch_param('invite_type') != 'incoming' )
		{
		...
		{
			$sql	.= " AND $column = '" . $this->EE->db->escape_str( $this->member_id ) . "'";
		}
		elseif ( $this->EE->session->userdata['member_id'] != 0 )
		{
			$sql	.= " AND $column = '" . $this->EE->db->escape_str( $this->EE->session->userdata['member_id'] ) . "'";
		}
		...
		if ( $this->EE->TMPL->fetch_param('member_group_id') !== FALSE AND
			 $this->EE->TMPL->fetch_param('member_group_id') != '' )
		{
			$sql	.= $this->EE->functions->sql_andor_string( $this->EE->TMPL->fetch_param( 'member_group_id' ), 'm.group_id' );
		}
		...
		if ( $this->EE->TMPL->fetch_param('email') !== FALSE AND $this->EE->TMPL->fetch_param('email') != '' )
		{
			$sql	.= " AND m.email = '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('email') ) . "'";
		}
		...
		if ( is_numeric( $this->EE->TMPL->fetch_param('days') ) )
		{
			...
			$sql	.= " AND f.entry_date >= '" . $this->EE->db->escape_str( $days ) . "'";
		}
		elseif ( is_numeric( $this->EE->TMPL->fetch_param('hours') ) )
		{
			$hours	= $this->EE->localize->now - ( $this->EE->TMPL->fetch_param('hours') + 3600 );

			$sql	.= " AND f.entry_date >= '" . $this->EE->db->escape_str( $hours ) . "'";
		}
		...
		if ( preg_match( "/\/(username|screen_name)\/(.+?)\//s", $this->EE->uri->uri_string, $match ) AND
			 $this->dynamic === TRUE )
		{
			$sql	.= ( $match['1'] == 'username' ) ? " AND m.username LIKE '": " AND m.screen_name LIKE '";
			$sql	.= $this->EE->db->escape_str( $match['2'] ) . "%'";
		...
		elseif ( $this->EE->TMPL->fetch_param('search_username') )
		{
			$sql	.= " AND m.username LIKE '" .
						$this->EE->db->escape_str( $this->EE->TMPL->fetch_param('search_username') ) . "%'";
		}
		elseif ( $this->EE->TMPL->fetch_param('search_screen_name') )
		{
			$sql	.= " AND m.screen_name LIKE '" .
						$this->EE->db->escape_str( $this->EE->TMPL->fetch_param('search_screen_name') ) . "%'";
		}
		...
		if ( $this->EE->TMPL->fetch_param('reciprocal') !== FALSE AND
			 $this->EE->TMPL->fetch_param('reciprocal') != '' )
		{
			if ( strpos( $this->EE->TMPL->fetch_param('reciprocal'), 'n' ) === FALSE )
			{
		...
		{
			if ( $this->EE->TMPL->fetch_param('show_blocked') === FALSE OR
				 ! $this->check_yes($this->EE->TMPL->fetch_param('show_blocked')) )
			{
				$sql	.= " AND f.block = 'n'";
			}
		}
		elseif ( $this->EE->TMPL->fetch_param('show_blocked') === FALSE OR
				 ! $this->check_yes($this->EE->TMPL->fetch_param('show_blocked')) )
		{
			//	----------------------------------------
			...
			$sql	.= " AND 	f.member_id
						 NOT IN ( SELECT 	member_id
								  ...
								  IN 		('".implode("','", $this->EE->TMPL->site_ids)."')
								  AND 		block = 'y'
								  AND 		friend_id = " . $this->EE->db->escape_str(
										$this->EE->session->userdata( 'member_id' ) ) . "
								)";
			...
			$sql	.= " AND 	f.member_id
						 NOT IN ( SELECT 	friend_id
								  ...
								  IN 		('".implode("','", $this->EE->TMPL->site_ids)."')
								  AND 		block = 'y'
								  AND 		member_id = " . $this->EE->db->escape_str(
										$this->EE->session->userdata( 'member_id' ) ) . "
								)";
		...
		if ( $this->EE->TMPL->fetch_param('orderby') !== FALSE AND
			 ...
			 strpos( $this->EE->TMPL->fetch_param('orderby'), '|' ) === FALSE )
		{
			if ( in_array( $this->EE->TMPL->fetch_param('orderby'), array( 'entry_date' ) ) === TRUE )
			{
				$sql	.= " ORDER BY f.".$this->EE->db->escape_str( $this->EE->TMPL->fetch_param('orderby') );
			}
			elseif ( $this->EE->TMPL->fetch_param('orderby') == 'random' )
			{
				$sql	.= " ORDER BY RAND() ";
			}
			else
		...
		if ( strtolower($this->EE->TMPL->fetch_param('sort')) != 'desc' )
		{
			$sql	.= " ASC";
		}
		else
		...
		$tdata	= ( $this->tagdata == '' ) ? $this->EE->TMPL->tagdata: $this->tagdata;
	...
	function _log( $msg )
	{
		...
		$this->EE->db->query($this->EE->db->insert_string('exp_cp_log', $data));
	...
	function members()
	{
		...
		if ( $this->_member_id() !== FALSE )
		{
			$sql	.= " AND m.member_id = '" . $this->EE->db->escape_str( $this->member_id ) . "'";
		}
		...
		if ( $this->EE->TMPL->fetch_param('friend_id') and is_numeric($this->EE->TMPL->fetch_param('friend_id')))
		{
			if ( $this->EE->TMPL->fetch_param('friend_id') == 'CURRENT_USER' )
			{
				$sql	.= $this->EE->functions->sql_andor_string( $this->EE->session->userdata['member_id'], 'm.member_id' );
			}
			else
			{
				$sql	.= $this->EE->functions->sql_andor_string( $this->EE->TMPL->fetch_param('friend_id'), 'm.member_id' );
			}
		}
	...
	function _member_id( $check_uri_for_raw_id = 'y' )
	{
		...
		if ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND $this->_numeric( $this->EE->TMPL->fetch_param('member_id') ) === TRUE )
		{
		...
		elseif ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND
				 $this->EE->TMPL->fetch_param('member_id') !== FALSE AND
				 $this->EE->TMPL->fetch_param('member_id') == 'CURRENT_USER' AND
		...
		elseif ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND
				 $this->EE->TMPL->fetch_param('username') !== FALSE AND
				 $this->EE->TMPL->fetch_param('username') != '' )
		{
			if ( $this->EE->TMPL->fetch_param('username') == 'CURRENT_USER' AND
				 $this->EE->session->userdata('member_id') != 0 )
			{
				$this->member_id	= $this->EE->session->userdata('member_id');
		...
		elseif ( isset($this->EE->TMPL) AND is_object($this->EE->TMPL) AND
				 $this->EE->TMPL->fetch_param('friend_username') !== FALSE AND
				 $this->EE->TMPL->fetch_param('friend_username') != '' )
		{
			if ( $this->EE->TMPL->fetch_param('friend_username') == 'CURRENT_USER' AND
				 $this->EE->session->userdata('member_id') != 0 )
			{
				$this->member_id	= $this->EE->session->userdata('member_id');
			...
			else
			{
				$query	= $this->EE->db->query(
					"SELECT member_id
					 FROM 	exp_members
					 WHERE 	username = '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('friend_username') ) . "'"
				);
		...
		elseif ( preg_match( "#/".$this->trigger."/(\w+)/?#", $this->EE->uri->uri_string, $match ) AND
				 $this->dynamic === TRUE )
		{
			$sql	= "SELECT 	member_id
					   FROM 	exp_members";
		...
		if ( $this->EE->uri->query_string != '' AND $this->dynamic === TRUE )
		{
			...
			else
			{
				...
				if (preg_match("#^".$cat_segment."/#", $qstring, $match) AND $this->EE->TMPL->fetch_param($this->sc->channel))
				{
					...
					{
						$sql	.= " AND {$this->sc->db->id} = '" . UB_BLOG_ID . "'";
					}
					else
					{
						$xsql	= $this->EE->functions->sql_andor_string(
	...
	function mine( $forced_friends = array() )
	{
		...
		{
			$sql	.= " AND f.member_id = '" . $this->EE->db->escape_str( $this->member_id ) . "'";
		}
		elseif ( $this->EE->session->userdata['member_id'] != 0 )
		{
			$sql	.= " AND f.member_id = '".$this->EE->db->escape_str($this->EE->session->userdata['member_id'])."'";
			$this->member_id	= $this->EE->session->userdata['member_id'];
		}
		else
		{
		...
		if ( $this->EE->TMPL->fetch_param('friend_id') !== FALSE AND $this->EE->TMPL->fetch_param('friend_id') != '' )
		{
			$sql	.= $this->EE->functions->sql_andor_string( $this->EE->TMPL->fetch_param('friend_id'), 'f.friend_id' );
		}
		...
		if ( $this->EE->TMPL->fetch_param('member_group_id') !== FALSE AND $this->EE->TMPL->fetch_param('member_group_id') != '' )
		{
			$sql	.= $this->EE->functions->sql_andor_string( $this->EE->TMPL->fetch_param( 'member_group_id' ), 'm.group_id' );
		}
		...
		if ( $this->EE->TMPL->fetch_param('reciprocal') !== FALSE AND $this->EE->TMPL->fetch_param('reciprocal') != '' )
		{
			if ( strpos( $this->EE->TMPL->fetch_param('reciprocal'), 'n' ) !== FALSE )
			{
				$sql	.= " AND f.reciprocal = 'n'";
		...
		if ( $this->EE->TMPL->fetch_param('show_blocked') !== FALSE AND
			 $this->check_yes($this->EE->TMPL->fetch_param('show_blocked')) )
		{
			//????
		}
		...
		if ( $this->check_no($this->EE->TMPL->fetch_param('public')) 	OR
			 $this->check_yes($this->EE->TMPL->fetch_param('private'))  	)
		{
			$sql	.= " AND f.private = 'y'";
		}
		...
		{
			$sql	.= " AND 	m.username
						 LIKE 	'" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('search_username') ) . "%'";
		}
		elseif ( $this->EE->TMPL->fetch_param('search_screen_name') )
		{
			$sql	.= " AND 	m.screen_name
						 LIKE 	'" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('search_screen_name') ) . "%'";
		}
		...
		if ( $this->EE->TMPL->fetch_param('birthday_range') !== FALSE AND
			 $this->EE->TMPL->fetch_param('birthday_range') != '' )
		{
			if ( $this->EE->TMPL->fetch_param('birthday_range') == '0' OR
				 $this->EE->TMPL->fetch_param('birthday_range') == 'today' )
			{
				$sql	.= " AND m.bday_m = " . date( 'n', $this->EE->localize->set_localized_time() );
				$sql	.= " AND m.bday_d = " . date( 'j', $this->EE->localize->set_localized_time() );
			}
			else
			{
				$valid_dates	= array();
				...
				for ( $i=0; $i<=$this->EE->TMPL->fetch_param('birthday_range'); $i++ )
				{
		...
		if ( $this->EE->TMPL->fetch_param('orderby') !== FALSE AND
			 ...
			 strpos( $this->EE->TMPL->fetch_param('orderby'), '|' ) === FALSE )
		{
			if ( $this->EE->TMPL->fetch_param('orderby') == 'birthday' )
			{
				$sql	.= " ORDER BY birthday";
			}
			else if (strtolower($this->EE->TMPL->fetch_param('orderby')) == 'random')
			{
				$is_random = TRUE;
				$sql	.= " ORDER BY RAND()";
			}
	...
	function mutual_friends()
	{
		...
		if ( $this->EE->TMPL->fetch_param( 'friend_id' ) !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param( 'friend_id' ) ) === TRUE )
		{
			$friend_id	= $this->EE->TMPL->fetch_param( 'friend_id' );
		}

		elseif ( $this->EE->TMPL->fetch_param( 'friend_username' ) !== FALSE AND
				 $this->EE->TMPL->fetch_param( 'friend_username' ) != '' )
		{
		...
		elseif ( preg_match( "#/"."friend_name"."/(\w+)/?#", $this->EE->uri->uri_string, $match ) AND
				 $this->dynamic === TRUE )
		{
			$friend_id	= $this->data->get_member_id_from_username( $this->EE->config->item( 'site_id' ), $match[1] );
		}
		...
		if ( $this->check_yes($this->EE->TMPL->fetch_param('reciprocal')) )
		{
			$reciprocal	= 'y';
		}

		if ( $this->check_no($this->EE->TMPL->fetch_param('reciprocal')) )
		{
			$reciprocal	= 'n';
		}
		...
		if ( $this->EE->session->userdata( 'member_id' ) != $this->member_id )
		{
		...
		$subquery2	= "SELECT 	friend_id
					   ...
					   AND 		site_id
					   IN	 	('" . implode("','", $this->EE->TMPL->site_ids) . "')
					   AND 		member_id = " . $this->EE->db->escape_str( $friend_id );
	...
	function _notify( $data = array() )
	{
		...
		$vars['from_email']			= ( $data['from_email'] ) ?
											$data['from_email'] : $this->EE->config->item('webmaster_email');
		$vars['from_name']			= ( $data['from_name'] ) ?
											$data['from_name']  : $this->EE->config->item('webmaster_name');
		$vars['friends_subject']	= ( isset( $data['subject'] ) === TRUE ) ?
										$data['subject'] :
										'Someone has added you as a friend at '.$this->EE->config->item('site_name');
		 ...
		 )
		{
			...
			$parse_custom_fields = $this->data->get_preference_from_site_id(
										$this->EE->config->item( 'site_id' ), 'notify_parse_all' ) == 'y';

			// There was a bug with not having a preference for this, so double check our setting
			if( ! $parse_custom_fields )
			{
				if( ! ($this->data->get_preference_from_site_id( $this->EE->config->item( 'site_id' ), 'notify_parse_all' ) == 'n' ) )
				{
			...
			if ( empty( $this->cache['members'][$data['member_id']] ) === TRUE )
			{
				$query	= $this->EE->db->query(
					"SELECT *
					 FROM 	exp_members
					 WHERE 	member_id = " . $this->EE->db->escape_str( $data['member_id'] )
				);
				...
				{
					$member_data_query	= $this->EE->db->query(
						"SELECT *
			...
			if ( empty( $this->cache['members'][ $this->EE->session->userdata('member_id') ] ) === TRUE )
			{
				$query	= $this->EE->db->query(
					"SELECT *
					 FROM 	exp_members
				...
				$this->cache['members'][ $this->EE->session->userdata('member_id') ]	= $row;
			}
			...
			foreach ( $this->cache['members'][ $this->EE->session->userdata('member_id') ] as $key => $val )
			{
				foreach ( array( 'friends_subject', 'friends_message' ) as $item )
				{
					//	----------------------------------------
		...
		$this->EE->email->wordwrap	= $this->check_yes($vars['wordwrap']);
		$this->EE->email->mailtype	= $this->check_yes($vars['html']) ? 'html': 'text';

		$this->EE->email->initialize();
		$this->EE->email->from( 		$vars['from_email'], $vars['from_name'] );
		$this->EE->email->to( 		$vars['recipient'] );
		$this->EE->email->subject( 	$vars['friends_subject'] );
		...
		$this->EE->db->query( $this->EE->db->insert_string( 'exp_friends_notification_log', $data ) );
	}
	...
	function _parse( $inject = array() )
	{
		...
		if ( $this->EE->TMPL->fetch_param('disable') AND stristr( $this->EE->TMPL->fetch_param('disable'), 'member_fields' ) )
		{
			$disable_member_fields	= TRUE;
		}

		if ( $this->EE->TMPL->fetch_param('disable') AND stristr( $this->EE->TMPL->fetch_param('disable'), 'date_fields' ) )
		{
		...
		$avatar_url		= $this->EE->config->item('avatar_url');
		$photo_url		= $this->EE->config->item('photo_url');
		$signature_url	= $this->EE->config->item('signature_url');
		...
		foreach ( $this->query->result_array() as $row )
		{
			...
			$tagdata	= ( $this->tagdata == '' ) ? $this->EE->TMPL->tagdata: $this->tagdata;
				...
				foreach ($dates as $val)
				{
					   ...
					   )
					{
						...
						foreach ( $codes as $code )
						{
							$str	= str_replace(
								$code,
								$this->EE->localize->convert_timestamp( $code, $row[$val], TRUE ),
								$str
							);
						}
				 ...
				 $disable_member_fields === FALSE )
			{
				foreach ( $this->mfields as $key => $val )
				{
					...
					if ( preg_match(
							...
							$match ) )
					{
						$data		= $this->EE->TMPL->fetch_data_between_var_pairs( $tagdata, 'select_' . $key );
	...
	function _parse_date( $format = '', $date = 0 )
	{
		...
		foreach ( $codes as $code )
		{
			$format	= str_replace( $code, $this->EE->localize->convert_timestamp( $code, $date, TRUE ), $format );
		}
	...
	function _parse_member_data( $member_id = '', $tdata = '', $prefix = 'friends_', $inject = array(), $include_counts = TRUE )
	{
		...
		if ( is_array( $member_id ) === FALSE AND isset( $this->cache['member_data'][$prefix.$member_id] ) === TRUE )
		{
		...
		else
		{
			$sql	=  "SELECT
						...
						m.daylight_savings 			AS {$prefix}daylight_savings,
						mg.group_title 				AS {$prefix}member_group_title, ";
			...
			{
				$sql	.= ' WHERE m.member_id = '.$this->EE->db->escape_str( $member_id );
			}
		...
		$disable_member_fields	= $this->check_yes($this->EE->TMPL->fetch_param('disable_member_fields'));
	...
	function preferences()
	{
		...
		if ( $this->EE->TMPL->fetch_param('exclude') !== FALSE AND
			 $this->EE->TMPL->fetch_param('exclude') != '' )
		{
		...
		{
			$tagdata	= $this->EE->functions->prep_conditionals( $this->EE->TMPL->tagdata, array( 'preference_value' => $val ) );
	...
	function _prep_pagination( $sql, $url_suffix = '' )
	{
		...
		if ( $this->EE->TMPL->fetch_param('limit') !== FALSE AND
			 $this->_numeric( $this->EE->TMPL->fetch_param('limit') ) !== FALSE )
		{
			$this->limit	= $this->EE->TMPL->fetch_param('limit');
		}

		$offset = ( ! $this->EE->TMPL->fetch_param('offset') OR
					! $this->_numeric($this->EE->TMPL->fetch_param('offset'))) ?
						'0' : $this->EE->TMPL->fetch_param('offset');
		...
		{
			$this->EE->TMPL->tagdata .= $this->paginate_match[0];
		}

		// We only want to engage full pagination if we have a
		// paginate block in the tagdata
		if ( strpos( $this->EE->TMPL->tagdata, 'paginate' . RD ) !== FALSE )
		{
			//get pagination info
		...
		{
			$sql .= ' LIMIT ' . $this->EE->db->escape_str($offset) . ',' . $this->EE->db->escape_str($this->limit);
	...
	public function _reciprocal( $members = array() )
	{
		...
		if ($this->EE->session->userdata('member_id') != 0 AND
			! in_array($this->EE->session->userdata('member_id'), $members))
		{
			$members[] = $this->EE->session->userdata('member_id');
		}
	...
	private function _reciprocal_single($member)
	{
		...
		$sql = "SELECT 	member_id, friend_id, entry_id
				FROM	exp_friends
				WHERE	block = 'n'
				AND 	(member_id = '" . $this->EE->db->escape_str($member) ."'
						 OR friend_id = '" . $this->EE->db->escape_str($member) ."')";
	...
	function _recursive_referral_check ( $primary_id, $referrer_id = 0 )
	{
		...
		$query	= $this->EE->db->query(
			"SELECT referrer_id
			 FROM 	exp_friends
			 WHERE 	referrer_id != '0'
			 AND 	friend_id = '" . $this->EE->db->escape_str( $friend_id ) . "'
			 LIMIT 	1"
		);
		...
		$query	= $this->EE->db->query(
			"SELECT COUNT(*) AS count
			 ...
			 WHERE 	member_id = '" . $this->EE->db->escape_str( $primary_id ) . "'
	...
	function referral_check ()
	{
		...
		if ( $this->EE->TMPL->fetch_param( 'interval' ) !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param( 'interval' ) ) === TRUE )
		{
				...
				$interval ) !== FALSE )
		{
			...
			$sql	= "SELECT 		m.member_id, m.email, f.entry_id, f.group_id
					   ...
					   AND 			f.referrer_id = " . $this->EE->db->escape_str( $this->EE->session->userdata('member_id') );
			...
			{
				$this->_add_friends_to_group( $member_ids, array(), $this->EE->session->userdata('member_id'), $group_id );
			}
			...
			$this->EE->db->query(
				...
				VALUES (
					...
					" . $this->EE->db->escape_str( $this->EE->session->userdata( 'member_id' ) ) . "
				)"
			);
		}
		...
		if ( $this->data->time_to_check_referrals_for_invitee(
				$this->EE->config->item('site_id'),
				$this->EE->session->userdata('member_id') ) !== FALSE
			)
		{
			//	----------------------------------------
			...
			$sql = "SELECT 		m.member_id, m.email, f.entry_id, f.group_id, f.referrer_id
					...
					AND 		f.referrer_id != 0
					AND 		m.member_id != ''
					AND 		f.email = '" . $this->EE->db->escape_str( $this->EE->session->userdata('email') ) . "'";
			...
			if ( ! $this->check_no($this->EE->TMPL->fetch_param( 'process_referrals' )) )
			{
				$this->_recursive_referral_check( $this->EE->session->userdata('member_id') );
			}
		}
	}
	...
	function referral_count ()
	{
		...
		$query	= $this->EE->db->query(
			"SELECT COUNT(*) AS friends_referral_count
			 FROM 	exp_friends_referrals
			 WHERE 	referrer_id = '" . $this->EE->db->escape_str( $this->member_id ) . "'"
		);
	...
	function search()
	{
		...
		if ( $this->EE->TMPL->fetch_param('friend_id') )
		{
			if ( $this->EE->TMPL->fetch_param('friend_id') == 'CURRENT_USER' )
			{
				$sql	.= $this->EE->functions->sql_andor_string( $this->EE->session->userdata['member_id'], 'f.friend_id' );
			}
			else
			{
				$sql	.= $this->EE->functions->sql_andor_string( $this->EE->TMPL->fetch_param('friend_id'), 'f.friend_id' );
		...
		elseif ( $this->EE->TMPL->fetch_param('username') )
		{
			$sql	.= " AND m.username LIKE '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('username') ) . "%'";
		}
		elseif ( $this->EE->TMPL->fetch_param('screen_name') )
		{
			$sql	.= " AND m.screen_name LIKE '" . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('screen_name') ) . "%'";
		}
		...
		if ( $this->EE->input->get_post('keywords') !== FALSE AND
			 $this->EE->input->get_post('keywords') != '' )
		{
			$sql		.= " AND (";
			...
			if ( $this->EE->TMPL->fetch_param('custom_fields') !== FALSE AND
				 $this->EE->TMPL->fetch_param('custom_fields') != '' )
			{
				$custom_fields	= preg_split( "/,|\|/", $this->EE->TMPL->fetch_param('custom_fields') );
				...
				foreach ( $custom_fields as $field )
				{
					if ( isset( $this->mfields[$field] ) === TRUE )
					{
						$sqls[]	= "`md.m_field_id_".$this->mfields[$field]['id']."` LIKE '%" .
									$this->EE->db->escape_str( $keywords ) . "%'";
					}
				}
			}

			$sqls[]	= "m.screen_name LIKE '%" . $this->EE->db->escape_str( $keywords ) . "%'";
		...
		if ( $this->EE->TMPL->fetch_param('exclude') !== FALSE AND $this->EE->TMPL->fetch_param('exclude') != '' )
		{
			$exclude	.= str_replace( ",", "|", $this->EE->TMPL->fetch_param('exclude') );
		}

		if ( $this->EE->input->get_post('exclude') !== FALSE AND $this->EE->input->get_post('exclude') != '' )
		{
			$exclude	.= str_replace( ",", "|", $this->EE->input->get_post('exclude') );
		}
		...
		if ( $this->EE->TMPL->fetch_param('orderby') )
		{
			...
			{
				$sql	.= " ORDER BY ".str_replace( "|", ",", $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('orderby')  ) );
			}
		}
		else
		{
		...
		{
			$sql	.= " LIMIT " . $this->EE->db->escape_str( $this->EE->TMPL->fetch_param('limit') );
		}
	...
	function _security()
	{
		...
		{
			if ( ( $this->EE->input->ip_address() == '0.0.0.0' OR
				   $this->EE->session->userdata['user_agent'] == '' ) AND
		...
		if ($this->check_yes($this->EE->config->item('ip2nation')) AND
			$this->EE->db->table_exists('ip2nation'))
		{
		...
		if ( $this->check_yes($this->EE->blacklist->blacklisted) AND
	...
	function profile_wall()
	{
		...
		if ( $this->EE->TMPL->fetch_param( 'friend_id' ) !== FALSE AND
			 ( is_numeric( $this->EE->TMPL->fetch_param( 'friend_id' ) ) === TRUE
				OR $this->EE->TMPL->fetch_param('friend_id') === 'CURRENT_USER' ) )
		{
			$friend_id	= $this->EE->TMPL->fetch_param( 'friend_id' );
		...
		elseif ( preg_match( "#/"."friend_username"."/(\w+)/?#", $this->EE->uri->uri_string, $match ) AND
				 $this->dynamic === TRUE )
		{
			$friend_id	= $this->data->get_member_id_from_username( $this->EE->config->item( 'site_id' ), $match[1] );
		}
		...
		$orderby			= ( ! in_array($this->EE->TMPL->fetch_param('orderby'), array(FALSE, '')) AND
								$this->column_exists($this->EE->TMPL->fetch_param('orderby'),
								 'exp_friends_profile_comments')) ?
										$this->EE->TMPL->fetch_param('orderby') : 'entry_date';

		$sort				= (strtolower($this->EE->TMPL->fetch_param('orderby')) === 'asc') ? 'ASC' : 'DESC';
		...
		$sql = "SELECT 	fpc.*
				...
				WHERE 	friend_id = '" . $this->EE->db->escape_str($friend_id) . "'";


		if (	 $this->EE->TMPL->fetch_param( 'author_id' ) !== FALSE AND
				 is_numeric( $this->EE->TMPL->fetch_param( 'author_id' ) ) === TRUE)
		{
			$sql .= " AND author_id = '" . $this->EE->db->escape_str($this->EE->TMPL->fetch_param( 'author_id' )) . "'";
		}
		...
		if ($orderby !== FALSE)
		{
			$sql .= ' ORDER BY ' . $this->EE->db->escape_str($orderby) . " $sort";
		}
		...
		foreach ($query->result_array() as $row)
		{
			...
			$row['member_id'] = $row['CURRENT_USER'] = $this->EE->session->userdata['member_id'];

			$td = $tagdata;

			//normal goodies :D
			$td = $this->EE->functions->prep_conditionals(
				$td,
				array(
			...
			foreach ($this->EE->TMPL->var_single as $key => $val)
			{
				...
				if(isset($row[$key]))
				{
					if ($is_date)
					{
						$row[$key] = $this->EE->localize->decode_date($date_format, $row[$key]);

						//have to swap out the real, full key
						$td = $this->EE->TMPL->swap_var_single($full_key, $row[$key], $td);
					}
					...
					//normal prep
					$td = $this->EE->functions->prep_conditionals($td, array($key => $row[$key]));

					$td = $this->EE->TMPL->swap_var_single($key, $row[$key], $td);
				}
	...
	function group_wall()
	{
		...
		$friends_group_id	= ( ! in_array($this->EE->TMPL->fetch_param('friends_group_id'), array(FALSE, '')) AND
								is_numeric($this->EE->TMPL->fetch_param('friends_group_id'))
							  ) ? $this->EE->TMPL->fetch_param('friends_group_id') : FALSE;

		$friends_group_name	= ( ! in_array($this->EE->TMPL->fetch_param('friends_group_name'), array(FALSE, ''))) ?
								$this->EE->TMPL->fetch_param('friends_group_name') : FALSE;
		...
		$member_id			= ( ! in_array($this->EE->TMPL->fetch_param('member_id'), array(FALSE, '')) AND
								is_numeric($this->EE->TMPL->fetch_param('member_id'))
							  ) ? $this->EE->TMPL->fetch_param('member_id') : FALSE;
		...
		$username			= ( ! in_array($this->EE->TMPL->fetch_param('username'), array(FALSE, ''))) ?
								$this->EE->TMPL->fetch_param('username') : FALSE;
		...
		{
			$sql .= " WHERE group_id = '" . $this->EE->db->escape_str($friends_group_id) . "'
		...
		else
		{
			$sql .= " WHERE group_id IN (
						...
						WHERE	group_name = '" . $this->EE->db->escape_str($friends_group_name) . "'
						AND		site_id		= '" . $this->EE->db->escape_str( $this->EE->config->item('site_id') ) . "'
						LIMIT 	1
					  )";
		}
		...
		{
			$sql .= " AND author_id = '" . $this->EE->db->escape_str($member_id) . "'";
		}
	...
	function group_wall_form()
	{
		...
		$this->params['require_ip']		= ($this->EE->TMPL->fetch_param('require_ip')) ? $this->EE->TMPL->fetch_param('require_ip') : '';

		//	----------------------------------------
		//	Are we establishing any required fields?
		//	----------------------------------------

		$this->params['ee_required']	= ($this->EE->TMPL->fetch_param('required')) ? $this->EE->TMPL->fetch_param('required') : '' ;

		//	----------------------------------------
		//	Are we notifying anyone?
		//	----------------------------------------

		$this->params['ee_notify']		= ($this->EE->TMPL->fetch_param('notify')) ? $this->EE->TMPL->fetch_param('notify') : '' ;

		//	----------------------------------------
		//	Are we using a notification template?
		//	----------------------------------------

		$this->params['template']		= ($this->EE->TMPL->fetch_param('template')) ?
											str_replace(SLASH, '/', $this->EE->TMPL->fetch_param('template')) :
											'default_template' ;


		$this->params['prevent_duplicates'] = $this->check_yes($this->EE->TMPL->fetch_param('prevent_duplicates')) ? 'yes' : 'no' ;

		// ----------------------------------------
		//	Parse conditional pairs
		// ----------------------------------------

		$cond['captcha']				= ( $this->check_yes($this->EE->config->item('captcha_require_members'))  OR
		...
		$hidden = array(
			'ACT'					=> $this->EE->functions->fetch_action_id('Friends', 'insert_group_wall_comment'),
			...
			'friends_group_id'		=> $this->EE->TMPL->fetch_param('friends_group_id')
		);
		...
		$this->hdata['RET']			= (isset($_POST['RET'])) ? $_POST['RET'] : $this->EE->functions->fetch_current_uri();

		$this->hdata['form_name']	= ( $this->EE->TMPL->fetch_param('form_name') ) ?
										$this->EE->TMPL->fetch_param('form_name') : 'group_wall_form';

		$this->hdata['id']			= ( $this->EE->TMPL->fetch_param('form_id') ) ?
										$this->EE->TMPL->fetch_param('form_id') : 'group_wall_form';
		...
		if ( $class = $this->EE->TMPL->fetch_param('form_class') )
		{
		...
		if ( $form_title = $this->EE->TMPL->fetch_param('form_title') )
		{
			$r	= str_replace( "<form", "<form title=\"".htmlspecialchars($form_title)."\"", $r );
		}

		if ($this->EE->extensions->active_hook('friends_group_wall_form_end') === TRUE)
		{
			$r = $this->EE->extensions->universal_call('friends_group_wall_form_end', $r);
   ...
   // --------------------------------------------------------------------
	...
	function profile_wall_form()
	{
		...
		$param_friend_id				= $this->EE->TMPL->fetch_param('friend_id');
		...
		$hidden = array(
			'ACT'					=> $this->EE->functions->fetch_action_id('Friends', 'insert_profile_wall_comment'),
			'URI'					=> ($this->EE->uri->uri_string == '') ? 'index' : $this->EE->uri->uri_string,
			'XID'					=> ( ! isset($_POST['XID'])) ? '' : $_POST['XID'],
			'return'				=> $this->_chars_decode(str_replace(SLASH, '/', $return)),
			'friend_id'				=> $friend_id
		...
		$this->hdata['form_name']	= ( $this->EE->TMPL->fetch_param('form_name') ) ?
										$this->EE->TMPL->fetch_param('form_name') : 'profile_wall_form';

		$this->hdata['id']			= ( $this->EE->TMPL->fetch_param('form_id') ) ?
										$this->EE->TMPL->fetch_param('form_id') : 'profile_wall_form';
		...
		if ($this->EE->extensions->active_hook('friends_profile_wall_form_end') === TRUE)
		{
			$r = $this->EE->extensions->universal_call('friends_profile_wall_form_end', $r);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
	...
	function insert_group_wall_comment()
	{
		...
		if ( $this->member_id == 0)
		{
			return $this->EE->output->show_user_error('general', array(lang('not_logged_in')));
		}
		...
		$this->EE->load->library('friends_groups');

		if ( ! $this->EE->friends_groups->_group_id())
		{
			return $this->EE->output->show_user_error('general', array(lang('group_not_found')));
		}

		$this->group_id = $this->EE->friends_groups->group_id;
		...
		if( ! $this->data->member_of_group( $this->EE->config->item( 'site_id' ), $this->member_id, $this->group_id ))
		{
			return $this->EE->output->show_user_error('general', array(lang('not_group_member')));
		}
		...
		{
			if ( ! $this->EE->input->post($required_field))
			{
		...
		$wall_comment =  trim($this->EE->security->xss_clean($this->EE->input->post('friends_wall_comment')));

		if ($this->check_yes($this->_param('prevent_duplicates')))
		{
			$dupe_sql 	= "SELECT 	author_id, comment
						   FROM 	exp_friends_group_comments
						   WHERE 	author_id = '" . $this->EE->db->escape_str($this->member_id) . "'
						   AND		comment   = '" . $this->EE->db->escape_str($wall_comment) . "'";

			$dupe_q 	= $this->EE->db->query($dupe_sql);

			if ($dupe_q->num_rows() > 0)
			{
				return $this->EE->output->show_user_error('submission', array(lang('duplicate_comment')));
			}
		}
		...
		{
			if ($this->check_yes($this->EE->config->item('captcha_require_members'))  OR
				( $this->check_no($this->EE->config->item('captcha_require_members')) AND
				  $this->EE->session->userdata('member_id') == 0))
			{
				if ( ! isset($_POST['captcha']) || $_POST['captcha'] == '')
				{
					return $this->EE->output->show_user_error('submission', lang('captcha_required'));
				}
				else
				{
					...
					if ($res->row('count') == 0)
					{
						return $this->EE->output->show_user_error('submission', lang('captcha_incorrect'));
					}
		...
		$sql			= $this->EE->db->insert_string( 'exp_friends_group_comments', $data );
		...
		if ( ! $return = $this->EE->input->get_post('return') )
		{
			$return	= $this->EE->input->get_post('RET');
		}
	...
	function insert_profile_wall_comment()
	{
		...
		$friend_id = $this->EE->input->post('friend_id');

		if ( ! is_numeric($friend_id) OR $friend_id <= 0)
		{
			return $this->EE->output->show_user_error('general', array(lang('member_not_found')));
		}
		...
		$members_friends = $this->data->get_friend_ids_from_member_id( $this->EE->config->item( 'site_id' ), $friend_id );

		if( $this->member_id != $friend_id AND ! in_array($this->member_id, $members_friends))
		{
			return $this->EE->output->show_user_error('general', array(lang('not_friends')));
		}
		...
		$sql			= $this->EE->db->insert_string( 'exp_friends_profile_comments', $data );
	...
	function profile_comment_delete()
	{
		...
		if ( ( $this->member_id = $this->EE->session->userdata('member_id') ) == 0 )
		{
			return $this->_fetch_error( lang('not_logged_in') );
		}
		...
		if ( $this->EE->TMPL->fetch_param( 'comment_id' ) !== FALSE AND
			 is_numeric( $this->EE->TMPL->fetch_param( 'comment_id' ) ) === TRUE )
		{
		...
		$comment_data 	= $this->data->get_data_from_profile_comment_id( $this->EE->config->item('site_id'), $comment_id );
		...
		if ($this->EE->extensions->active_hook('friends_profile_comment_delete') === TRUE)
		{
			$data	= $this->EE->extensions->universal_call( 'friends_profile_comment_delete', $this, $data );
			if ( $this->EE->extensions->end_script === TRUE ) exit();
		}
		...
		$this->EE->db->query(
			"DELETE FROM exp_friends_profile_comments
			 WHERE 		 comment_id = " . $this->EE->db->escape_str( $comment_id )
		);
		...
		$tagdata	= $this->EE->functions->prep_conditionals( $this->EE->TMPL->tagdata, $data );
	...
	function group_comment_delete()
	{
		...
		$is_admin 		= ($this->EE->session->userdata('group_id') == 1);
		$comment_data 	= $this->data->get_data_from_group_comment_id( $this->EE->config->item('site_id'), $comment_id );
		...
		if ($this->EE->extensions->active_hook('friends_group_comment_delete') === TRUE)
		{
			$data	= $this->EE->extensions->universal_call( 'friends_group_comment_delete', $this, $data );
			if ( $this->EE->extensions->end_script === TRUE ) exit();
		}
	...
	function update()
	{
		...
		$this->delete	= $this->check_yes($this->EE->input->post('friends_delete'));

		//	----------------------------------------
		//	Are we deleting?
		//	----------------------------------------

		$block_mode		= $this->check_yes($this->EE->input->post('friends_block'));

		//	----------------------------------------
		//	Are we notifying?
		//	----------------------------------------

		$this->notify	= $this->check_yes($this->EE->input->post('friends_notify'));
		...
		if ( strpos( $this->EE->uri->uri_string, "/delete" ) !== FALSE OR $this->delete === TRUE )
		{
		...
		elseif ( strpos( $this->EE->uri->uri_string, "/block" ) !== FALSE OR $block_mode === TRUE )
		{
			if ( $this->_block( $members ) === FALSE )
			{
				return $this->_fetch_error( $this->message );
		...
		{
			return $this->EE->output->show_message(
				array(
					'title' 	=> lang('success'),
		...
		if ( count( $this->params ) == 0 )
		{
			...
			$query	= $this->EE->db->query(
				"SELECT data
				 ...
				 WHERE 	params_id = '" . $this->EE->db->escape_str( $this->params_id ) . "'"
			);
			...
			$this->EE->db->query(
				"DELETE FROM 	{$this->params_tbl}
				 WHERE 			entry_date < " . $this->EE->db->escape_str( ($this->EE->localize->now - 7200) )
			);
		}
