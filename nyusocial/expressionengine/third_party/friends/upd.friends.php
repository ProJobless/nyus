...
class Friends_upd extends Module_builder_friends
{
	...
	public function install()
	{
		...
		$this->EE->db->query("DELETE FROM exp_extensions WHERE class = 'Friends_ext'");
	...
	public function uninstall()
	{
		...
		{
			$this->EE->db->query("ALTER TABLE exp_members DROP `total_public_friends`");
		}
	...
	public function update()
	{
		...
		if ($this->version_compare($this->database_version(), '<', '1.5.6'))
		{
			...
			$this->actions()->delete_members_from_friends();
		}
	...
	function _sql_friends()
	{
		...
		if ( $this->EE->db->table_exists( 'exp_friends' ) )
		{
	...
	function _sql_group_posts()
	{
		...
		if ( $this->EE->db->table_exists( 'exp_friends_group_posts' ) )
		{
	...
	function _sql_groups()
	{
		...
		if ( $this->EE->db->table_exists( 'exp_friends_groups' ) )
		{
