 ...
 */
 ...
 // MAJOR HACKING!
 // _upload_images() - added global flag checking for profile photo uploaded via POST ~ line 11571
...
class User extends Module_builder_user
{
	...
	public function edit_profile()
	{
		...
		// saving the raw data from xss cleaning
		$temp_profile   = isset($_POST["photo_filename"]["data"])   ? $_POST["photo_filename"]["data"] : "";
		
		$_POST	= $this->EE->security->xss_clean( $_POST );

		if ($temp_profile != '')
		{
    		$_POST["photo_filename"]["data"] = $temp_profile;
		}
	...
	private function _upload_images( $member_id = 0, $test_mode = FALSE )
	{
		...
		// BEW
		// 05.09.13
		// We are using a FileReader polyfill to show image previews on older browsers
		// The FileReader hijacks the file upload process, so photos are submitted via POST
		// We're adding a Global flag to signify later that we're also looking for the image in a POST subarray
		//
	...
	private function _upload_image( $type = 'avatar', $member_id = 0, $test_mode = TRUE )
	{
		...
		if (isset($GLOBALS["flag"])) {
    		$size = ceil(($_POST[$this->uploads[$type]]['size']/1024));
		} else {
		...
		if (isset($GLOBALS["flag"])) {
    		$filename = $_POST[$this->uploads[$type]]["name"];
		} else {
		...
		if ($this->EE->upload->do_upload($this->uploads[$type]) === FALSE && !isset($GLOBALS["flag"]))
		{
        ...
        if (isset($GLOBALS["flag"])) {            
            $imageData = $_POST["photo_filename"]["data"];
            $string = file_get_contents($imageData);
            $image = imagecreatefromstring($string);        
            imagejpeg($image, $upload_path.$new_filename, 70);
            
            $imageSize = getimagesize($upload_path.$new_filename);
            $width = $imageSize[0];
            $height = $imageSize[1];
		} else {
